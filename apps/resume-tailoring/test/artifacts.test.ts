...
describe("run input artifacts", () => {
  test("uses exactly data/runs/{queue-sequence}/input and rejects invalid run sequences", async () => {
    ...
    expect(store.inputRoot(address)).toBe(join(root, "123", "input"));
    for (const run of [0, -1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
      expect(() => store.inputRoot({ run })).toThrow(/invalid run/i);
    }

    const input = await store.createRunInput(address);
    expect(input).toBe(join(root, "123", "input"));
  ...
  test("creates only real owner-private directories and refuses symlinked paths", async () => {
    ...
    await symlink(outside, join(root, "456"));

    await expect(store.createRunInput({ run: 456 })).rejects.toThrow(/symlink/i);

    const input = await store.createRunInput(address);
    if (process.platform !== "win32") {
      expect((await lstat(root)).mode & 0o777).toBe(0o700);
      expect((await lstat(join(root, String(address.run)))).mode & 0o777).toBe(0o700);
  ...
  test("removes only one explicit numeric run tree and treats absence as success", async () => {
    const root = join(await artifactRoot(), "data", "runs");
    const store = new ArtifactStore(root);
    const input = await store.createRunInput({ run: 789 });
    ...
    await expect(store.removeRun(789)).resolves.toBeTrue();
    await expect(lstat(join(root, "789"))).rejects.toMatchObject({ code: "ENOENT" });
    expect((await lstat(root)).isDirectory()).toBeTrue();
    expect(await readFile(join(unknown, "keep.txt"), "utf8")).toBe("keep");
    await expect(store.removeRun(789)).resolves.toBeFalse();
  });

  test("treats an overlapping removal that loses the lstat-to-rm race as absent", async () => {
    const store = new ArtifactStore(join(await artifactRoot(), "data", "runs"));
    await store.createRunInput({ run: 321 });
  ...
  test("rejects unsafe run roots without following descendant symlinks", async () => {
    ...
    await symlink(outside, join(root, "654"));
    await writeFile(join(root, "655"), "not a directory");

    for (const run of [0, -1, 1.25, Number.NaN, Number.POSITIVE_INFINITY]) {
      await expect(store.removeRun(run)).rejects.toThrow(/invalid run/i);
    }
    await expect(store.removeRun(654)).rejects.toThrow(/real directory/i);
    await expect(store.removeRun(655)).rejects.toThrow(/real directory/i);

    const input = await store.createRunInput({ run: 987 });
    await symlink(outside, join(input, "outside-link"));
    await expect(store.removeRun(987)).resolves.toBeTrue();
