...
const JOB_NARRATIVE_POLICY = "For every job-specific short-answer, textarea, or open-ended why/how/describe prompt, call request_additional_info with answer_type \"text\" before fill/type; never compose, infer, revise, or reuse text. Enter only the exact current-session answer, then re-inspect without re-asking. If unanswered, leave optional text blank or re-ask if required. Deterministic supplied profile/contact and fixed-choice/boolean fields are excluded.";

const EXPECTED_REQUEST_ADDITIONAL_INFO_DESCRIPTION = "After a successful browser inspection, fill every visible field supported by current facts except the job narrative fields defined below, and upload the supplied resume when visible. Then ask one bounded batch for remaining visible fields whose facts are unavailable. For job applications, every application-specific open-ended narrative/free-text prompt—including any short answer, textarea, or why/how/describe prompt—must be included with answer_type \"text\" before any fill or type, even when profile context or a saved answer seems usable; batch all currently visible prompts that lack accepted current-session answers. After an accepted current-session answer for the exact question, enter it exactly and do not ask again. A continue or decline without an answer never permits manufactured text. Scope reusable availability globally and job-source or referral facts per application. Use lowercase snake_case question and option IDs, and lowercase dot-separated snake_case keys. Do not use this for browser interaction. Treat a deterministic question as already answered by current facts unless the page conflicts; treat a job narrative question as answered only after its accepted current-session response.";
