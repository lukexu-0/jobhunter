    ...
    manager = ApplicationSessionManager(
        ...
        browser_launches=(
            ...
            ResolvedBrowserLaunch(
                cdp_url=None,
                executable_path=tmp_path / "fake-chrome",
                user_data_dir=tmp_path / "profile-2",
            ),
            ResolvedBrowserLaunch(
                cdp_url=None,
                executable_path=tmp_path / "fake-chrome",
                user_data_dir=tmp_path / "profile-3",
            ),
        ),
...
async def test_preflight_completes_before_playwright_runtime_and_create_contract_is_public_safe(
    ...
    await wait_until(lambda: len(next(iter(manager._active.values())).events) >= 2)
    record = next(iter(manager._active.values()))
    assert record is not None
...
async def test_application_session_capacity_blocks_fourth_and_reuses_slot(
    tmp_path: Path,
) -> None:
    manager, _fakes, _root = make_manager(tmp_path, blocked_runner)
    first = await create_valid(manager)
    second = await create_valid(manager)
    third = await create_valid(manager)
    assert [call["launch"].user_data_dir for call in _fakes.runtime_factory_calls] == [
        tmp_path / "profile",
        tmp_path / "profile-2",
        tmp_path / "profile-3",
    ]
    ...
    await manager.delete(second.session_id)
    replacement = await create_valid(manager)
    ...
    for session_id in (first.session_id, third.session_id, replacement.session_id):
...
async def test_direct_values_literal_and_url_encoded_are_redacted_from_paths(
    ...
    await wait_until(
        lambda: bool(manager._active) and len(manager._active[created.session_id].events) >= 2
    )
    record = manager._active[created.session_id]
    assert record is not None
    public = json.dumps(
...
async def test_create_session_uses_latex_source_as_resume_evidence_and_pdf_for_upload(
    ...
    record = manager._active[session_id]
    assert record is not None
