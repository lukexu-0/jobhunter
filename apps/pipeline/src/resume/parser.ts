...
export const EDITABLE_SECTIONS = ["experience", "projects", "technical-skills"] as const;
...
export interface BaselineBullet { readonly id: string; readonly entityId: string; readonly section: "experience" | "projects"; readonly text: string; }
export interface BaselineEntity {
  readonly id: string;
  readonly entityId: string;
  readonly section: "experience" | "projects";
...
function parseEntities(region: ResumeRegion): BaselineEntity[] {
  ...
  const macro = section === "experience" ? "resumeSubheading" : "resumeProjectHeading";
  const argCount = section === "experience" ? 4 : 2;
  ...
  return headings.map((heading, index) => {
    ...
    const entityId = section === "experience" ? plain(headingArgs[2] ?? "") : plain(headingArgs[0] ?? "").split(/\s*\$\|\$\s*/)[0]!.trim();
...
export function parseBaselineResume(source: string): ParsedBaselineResume {
  ...
  const regions = { experience, projects, "technical-skills": technicalSkills } as const;
