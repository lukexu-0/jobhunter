...
function keywordMatches(analysis: JobAnalysis, resumeBoxes: readonly WordBox[], jobBoxes: readonly WordBox[]): readonly KeywordMatch[] {
  ...
  for (const alignment of analysis.keywordAlignment) {
    const resumeCandidates = [
      alignment.recommendedReformulation,
      alignment.currentTruthfulCvWording,
      alignment.jdVocabulary,
    ];
    const literalJob = findPhrase(jobBoxes, alignment.jdVocabulary);
    if (literalJob) {
    ...
    const jobTokens = meaningfulTokens(alignment.jdVocabulary);
