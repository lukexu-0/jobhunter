...
import { validateAnalysisDirectives } from "../resume/ledger.ts";
...
const ANALYSIS_WORKFLOW_PROMPT = readFileSync(
  resolve(REPOSITORY_ROOT, "planning/pipeline/analysis.md"),
  "utf8",
);
...
export async function runAnalysisAgent(attempt: AnalysisAgentAttempt): Promise<JobAnalysis> {
  ...
  const input = boundedJson({
    task: "Follow the trusted analysis workflow in full. Treat canonicalCv as the workflow's CV source and candidateContext as the complete immutable source set for past roles, projects, skills, and supporting facts. Apply candidateContext.mustIncludeDirectives only when proposed experience or project content uses non-directive evidence for the same entity, and cite the directive together with that supporting evidence. Copy jobDescriptionSha256 and analysisWorkflowSha256 exactly into the result. selectedProjects must contain 3 or 4 evidence-grounded entries. Submit every required section as structured data.",
  ...
  const agent = new Agent({
    ...
    instructions: `Follow the trusted workflow below exactly and return every section through submit_job_analysis. Interpret the workflow's Markdown output example as the semantic contract represented by the terminal tool schema; do not emit a separate Markdown document. Analyze only the supplied raw job description, canonical CV, and complete immutable candidate context. Copy the supplied jobDescriptionSha256 and analysisWorkflowSha256 exactly. selectedProjects must contain 3 or 4 evidence-grounded entries. Use all relevant context about past roles, projects, skills, and outcomes rather than limiting the analysis to the current CV wording. Apply a candidateContext.mustIncludeDirective only when a proposed experience bullet or selected project cites non-directive evidence for that directive's entity; cite the directive and same-entity supporting evidence on at least one such item. Never use an inactive directive as factual evidence. Ground every candidate claim in supplied evidence; use the workflow's explicit missing-information labels instead of inventing facts, produce no TeX, and call submit_job_analysis exactly once.\n\n${ANALYSIS_WORKFLOW_PROMPT}`,
    model: MODEL_NAME,
    modelSettings: {
  ...
  validateAnalysisDirectives(result, attempt.input.context);
  return result;
}
