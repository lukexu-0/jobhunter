...
export async function runRepairAgent(attempt: RepairAgentAttempt): Promise<RepairResult> {
  ...
  const modelInput = boundedJson({
    task: "Repair only TeX syntax, escaping, or existing macro-call argument syntax without changing visible content.",
  ...
  const agent = new Agent({
    ...
    instructions: "Use only the fixed failed TeX and log artifacts. Do not change visible content, order, links, evidence, sections, preamble, wrapper, or control-sequence order. Validate before compiling, remain within three candidate compiles, and call submit_repair exactly once.",
    model: MODEL_NAME,
    modelSettings: {
  ...
  return validateRepairResult(result, attempt.input.failedTex, attempt.input.canonicalBaseline);
}
