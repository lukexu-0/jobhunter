...
const address = { run: "run-1", revision: "revision-1", stage: "compile", attempt: 1 } as const;
...
describe("immutable artifacts", () => {
  test("contains run/revision/attempt paths and rejects traversal components", async () => {
    const store = new ArtifactStore(await root());
    expect(store.attemptRoot(address)).toEndWith("run-1/revisions/revision-1/attempts/compile-1");
    expect(() => store.attemptRoot({ ...address, run: "../escape" })).toThrow(/invalid run/i);
...
describe("trusted resume compiler", () => {
  ...
  test("classifies only syntax, brace, escaping, and macro-call failures as repairable", async () => {
    const cases = [
