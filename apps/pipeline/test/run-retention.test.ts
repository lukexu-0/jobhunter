...
async function persistedReviewWindow(ids: readonly string[]) {
  ...
  const records: { id: string; pdf: { id: string; path: string; byteSize: number; content: string } }[] = [];
  for (const id of ids) {
    const inputRoot = await artifacts.createRunInput({ run: id });
    const input = await artifacts.write(join(inputRoot, "job-description.txt"), `job:${id}`, 1024);
    repository.createQueuedRun(`job:${id}`, SOURCE_SNAPSHOT, {
      sha256: input.sha256,
      path: input.path,
    ...
    records.push({ id, pdf: { id: pdf.id, path: pdf.path, byteSize: pdf.byteSize, content } });
  }
...
test("continues after deletion failures and leaves failed reservations retryable", async () => {
  ...
  const artifacts = {
    async removeRun(runId: string): Promise<boolean> {
      if (runId === "run-a" && failRunA) throw new Error("injected deletion failure");
      return runId !== "run-b";
    },
  };
...
test("prunes only the two oldest inactive run trees while preserving all SQLite history", async () => {
  ...
  expect(existsSync(join(artifacts.root, ids[0]!))).toBeFalse();
  expect(existsSync(join(artifacts.root, ids[1]!))).toBeFalse();
...
test("temporarily keeps an old active run and prunes it after it becomes inactive", async () => {
  ...
  await expect(enforceRunArtifactRetention(repository, artifacts)).resolves.toBe(1);
  expect(existsSync(join(artifacts.root, ids[0]!))).toBeTrue();
  expect(existsSync(join(artifacts.root, ids[1]!))).toBeFalse();
