...
function nativePlaywrightCliGroup(callId: string, dt = true): readonly [
  ...
  return [
    ...
    {
      ...visibleCall,
      providerData: { jobhuntCodex: { version: 1, kind: "covered" } },
    },
...
describe("projectApplicationHistory", () => {
  test("uses separate stored-transcript and model-input budgets", () => {
    expect(MAX_APPLICATION_AGENT_TRANSCRIPT_BYTES).toBe(100 * 1024 * 1024);
    expect(MAX_APPLICATION_MODEL_INPUT_BYTES).toBe(10 * 1024 * 1024);
  });

  test("preserves Gemini thought signatures through projection without admitting them to Codex", () => {
    const history: AgentInputItem[] = [
      user("inspect the form"),
      { type: "reasoning", id: "reasoning-signature", content: [{ type: "input_text", text: "Inspect first." }] },
      { role: "assistant", status: "completed", content: [{ type: "output_text", text: "Checking the form.", providerData: { textSignature: "text-signature" } }] },
      { ...playwrightCliCall("signed-call"), providerData: { thoughtSignature: "tool-signature" } },
      playwrightCliResult("signed-call"),
    ];

    expect(projectApplicationHistory(history, "google-antigravity")).toEqual(history);
    expect(() => projectApplicationHistory(history)).toThrow(ApplicationHistoryProjectionError);
  });

  test("rejects foreign native history and invalid Gemini tool signatures", () => {
    const rejected: AgentInputItem[] = [
      nativePlaywrightCliGroup("foreign")[0],
      { type: "compaction", encrypted_content: "foreign-compaction" },
      { ...playwrightCliCall("empty"), providerData: { thoughtSignature: "" } },
      { ...playwrightCliCall("invalid"), providerData: { thoughtSignature: 1 } },
      { ...playwrightCliCall("unknown"), providerData: { thoughtSignature: "signed", extra: true } },
    ];
    for (const item of rejected) {
      expect(() => projectApplicationHistory([item], "google-antigravity"))
        .toThrow(ApplicationHistoryProjectionError);
    }
  });

  test("prunes old Gemini browser pairs while retaining signed narrative and unrelated tool exchanges", () => {
    const reasoning: AgentInputItem = { type: "reasoning", id: "thinking-signature", content: [{ type: "input_text", text: "The user chose this answer." }] };
    const narrative: AgentInputItem = { role: "assistant", status: "completed", content: [{ type: "output_text", text: "Keep the accepted answer.", providerData: { textSignature: "narrative-signature" } }] };
    const savedCall = { ...otherCall("saved-answer"), providerData: { thoughtSignature: "saved-signature" } };
    const savedResult = otherResult("saved-answer");
    const newerPairs: AgentInputItem[] = [];
    for (let index = 0; index < 16; index += 1) {
      newerPairs.push(
        { ...playwrightCliCall(`new-${index}`), providerData: { thoughtSignature: `signature-${index}` } },
        playwrightCliResult(`new-${index}`),
      );
    }
    const history = [
      reasoning, narrative,
      { ...playwrightCliCall("old"), providerData: { thoughtSignature: "old-signature" } },
      savedCall, playwrightCliResult("old"), savedResult,
      ...newerPairs,
    ];

    expect(projectApplicationHistory(history, "google-antigravity")).toEqual([
      APPLICATION_HISTORY_PRUNED_NOTICE, reasoning, narrative, savedCall, savedResult, ...newerPairs,
    ]);
  });
  ...
  test("treats the latest non-delta native Codex history as a replacement window", () => {
    const obsoletePrefix = user("x".repeat(MAX_APPLICATION_MODEL_INPUT_BYTES));
  ...
  test("fails with the dedicated provider error when preserved non-browser history overflows", () => {
    const oversized = user("x".repeat(MAX_APPLICATION_MODEL_INPUT_BYTES));
