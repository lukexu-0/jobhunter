...
export const AUTH_PROVIDERS = ["openai-codex", "google-antigravity"] as const satisfies readonly ApplicationModelProvider[];
...
export const authDatabasePath = resolve(import.meta.dir, import.meta.dir.endsWith(`${sep}dist`) ? "../../data/oauth/auth.sqlite" : "data/oauth/auth.sqlite");
let factory: StorageFactory = defaultStorageFactory;
let configuredAuthDatabasePath = authDatabasePath;
let storagePromise: Promise<AuthStorageLike> | undefined;

function isProvider(value: string): value is ApplicationModelProvider {
...
function validateOAuthRow(row: StoredAuthCredential): void {
  ...
  if (row.provider === "google-antigravity" && (typeof credential.projectId !== "string" || !credential.projectId)) {
    throw new AuthConfigurationError("Google Antigravity OAuth credential has no project identity");
  }
}

export function assertOAuthOnlyStorage(storage: AuthStorageLike): void {
  const rows = storage.listStoredCredentials();
  const counts = new Map<ApplicationModelProvider, number>();
  for (const row of rows) {
    validateOAuthRow(row);
    const provider = row.provider as ApplicationModelProvider;
...
export function assertProviderOAuthConnected(storage: AuthStorageLike, provider: ApplicationModelProvider): void {
...
async function createStorage(): Promise<AuthStorageLike> {
  const configuredPath = configuredAuthDatabasePath;
...
export function configureAuthDatabasePath(path: string): void {
  if (storagePromise) throw new Error("Cannot change auth storage after initialization");
  configuredAuthDatabasePath = path;
}

export function getAuthStorage(): Promise<AuthStorageLike> { storagePromise ??= createStorage().catch((error) => {
