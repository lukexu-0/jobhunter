...
export const ApplicationResultBaseSchema = z.object({
  ...
  files_attached: z
    .array(z.string().refine(isSanitizedBasename))
    .max(20)
    .default([])
    .describe("Exact task display_name values for documents uploaded in this application."),
...
export const GetCredentialsRuntimeActionSchema = z.object({
  type: z.literal("get_credentials"),
}).strict();
export type GetCredentialsRuntimeAction = z.infer<
  typeof GetCredentialsRuntimeActionSchema
>;
...
export const ReadInboxRuntimeActionSchema = z.object({
  ...
  query: z.string()
    ...
    .refine((value) => hasCodePointLength(value, 0, 500)
...
export const ReadUserInfoRuntimeActionSchema = z.object({
  type: z.literal("read_user_info"),
}).strict();
export type ReadUserInfoRuntimeAction = z.infer<
  typeof ReadUserInfoRuntimeActionSchema
>;
...
const CredentialValueSchema = z.string().refine((value) => (
  hasOnlyPairedUtf16Surrogates(value) && hasCodePointLength(value, 1, 4_096)
));

export const CredentialsRuntimeActionResponseSchema = z.object({
  type: z.literal("credentials"),
  username: CredentialValueSchema,
  password: CredentialValueSchema,
}).strict();
export type CredentialsRuntimeActionResponse = z.infer<
  typeof CredentialsRuntimeActionResponseSchema
>;
...
export const ReadUserInfoRuntimeActionResponseSchema = z.object({
  type: z.literal("read_user_info_result"),
  content: z.string().refine(hasOnlyPairedUtf16Surrogates),
}).strict();
export type ReadUserInfoRuntimeActionResponse = z.infer<
  typeof ReadUserInfoRuntimeActionResponseSchema
>;

export const GmailUnavailableRuntimeActionResponseSchema = z.object({
  type: z.literal("gmail_unavailable"),
  message: z.string().refine((value) => (
    hasOnlyPairedUtf16Surrogates(value) && hasCodePointLength(value, 1, 500)
  )),
}).strict();
export type GmailUnavailableRuntimeActionResponse = z.infer<
  typeof GmailUnavailableRuntimeActionResponseSchema
>;
...
const ERROR_MESSAGES: Readonly<Record<ApplicationRuntimeErrorCode, string>> = {
  ...
  invalid_response: "The runtime response is invalid",
};
...
async function readBoundedJson(response: Response, signal: AbortSignal): Promise<unknown> {
  if (response.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") {
    ...
    throw new ApplicationRuntimeError("invalid_response");
  }
  ...
  if (!reader) throw new ApplicationRuntimeError("invalid_response");
