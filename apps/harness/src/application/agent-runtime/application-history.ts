...
export const MAX_APPLICATION_AGENT_TRANSCRIPT_BYTES = 100 * 1024 * 1024;
export const MAX_APPLICATION_MODEL_INPUT_BYTES = 10 * 1024 * 1024;
...
export function projectApplicationHistory(
  ...
  modelProvider: "openai-codex" | "google-antigravity" = "openai-codex",
): AgentInputItem[] {
  ...
  for (const item of history) {
    ...
    try {
      ...
      } else if (item.providerData !== undefined) {
        const keys = Object.keys(item.providerData);
        if (keys.length !== 0 && (
          item.type !== "function_call"
          || keys.length !== 1
          || keys[0] !== "thoughtSignature"
          || typeof item.providerData.thoughtSignature !== "string"
          || item.providerData.thoughtSignature.length === 0
        )) throw new ApplicationHistoryProjectionError();
      }
    } catch {
      throw new ApplicationHistoryProjectionError();
  ...
  for (const [callIndex, resultIndex] of retainedPairs) {
    if (projectedBytes <= MAX_APPLICATION_MODEL_INPUT_BYTES) break;
    if (!pruned) {
      pruned = true;
  ...
  if (projectedBytes > MAX_APPLICATION_MODEL_INPUT_BYTES) {
    throw new ApplicationHistoryProjectionError();
  }
