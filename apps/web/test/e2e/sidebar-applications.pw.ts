...
async function expectFolderNavigation(
  page: Page,
  width: number,
  currentLabel: "Applications" | "Events" | "Providers",
): Promise<void> {
  ...
  expect(Math.abs(linkBoxes[1].x - linkBoxes[0].width)).toBeLessThanOrEqual(1);
  expect(Math.abs(linkBoxes[2].x - linkBoxes[0].width - linkBoxes[1].width)).toBeLessThanOrEqual(1);
  expect(Math.abs(
...
test("keeps dashboard snapshots visible while revalidating between Applications and Providers", async ({ page }) => {
  ...
  await expect(primaryNavigation.getByRole("link")).toHaveText(["Applications", "Events", "Providers"]);
