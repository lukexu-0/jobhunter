...
import { PipelineRepository } from "../resume-tailoring/src/db/repository.ts";
import { openPipelineDatabase } from "../resume-tailoring/src/db/database.ts";
import {
  restorePrunedRunArtifacts,
  type RunArtifactRestoreSummary,
} from "../resume-tailoring/src/system/run-artifact-restore.ts";
      ...
      summary = await withStoppedPipeline(configuration, () => {
        ...
        try {
          ...
          return restorePrunedRunArtifacts(repository, {
            ...
            priorArtifactRoot: configuration.priorArtifactRoot,
          });
        } finally {
          database.close();
        }
