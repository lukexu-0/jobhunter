...
const TRANSCRIPT_BYTES = new TextEncoder().encode("%PDF-1.7\nofficial transcript\n%%EOF\n");
...
class FakeHarness implements ApplicationHarnessClient {
  ...
  async create(input: ApplicationHarnessCreateInput): Promise<void> {
    this.createCalls.push({
      ...
      ...(input.transcriptPdf === undefined
        ? {}
        : { transcriptPdf: Uint8Array.from(input.transcriptPdf) }),
    });
    if (this.createError) {
...
async function createApprovedRun(
  ...
  const attempt = repository.startAttempt(claim, "compiling");
  ...
  repository.transition(claim, "deterministic_qa");
  for (let retry = 0; retry < (options.qaRetries ?? 0); retry += 1) {
    const qa = repository.startAttempt(claim, "deterministic_qa");
    repository.finishAttempt(claim, qa.id, "failed");
    repository.transition(claim, "failed", { failedStage: "deterministic_qa" });
    repository.release(claim);
    repository.retry(run.id);
    claim = repository.acquire();
    if (!claim || claim.runId !== run.id) throw new Error("retry claim missing");
  }
  repository.transition(claim, "visual_qa");
...
async function createTarget(options: {
  ...
  transcriptReader?: () => Uint8Array | undefined | Promise<Uint8Array | undefined>;
  sessionIds?: string[];
  ...
  const { pdf, run, tex } = await createApprovedRun(repository, database, artifacts, {
    ...
    ...(options.qaRetries === undefined ? {} : { qaRetries: options.qaRetries }),
  ...
  const service = new ApplicationSessionService({
    ...
    transcriptReader: options.transcriptReader ?? (() => undefined),
...
async function createProfileRoot(): Promise<{ root: string; profilePath: string }> {
  ...
  const profilePath = join(
    root,
    ".jobhunt-data/user-info/current-context/personal/applicant-profile.md",
  );
...
describe("application session service", () => {
  ...
  test("starts with the verified resume inherited through repeated QA retries", async () => {
    const target = await createTarget({
      approved: false,
      qaRetries: 2,
      transcriptReader: () => TRANSCRIPT_BYTES,
    });

    expect(await target.service.get(target.runId)).toEqual({
      state: "not_started", canStart: false, canStartAfterApproval: true,
    });
    target.repository.approve(target.runId, target.pdf.sha256);
    expect(await target.service.get(target.runId)).toEqual({
      state: "not_started", canStart: true, canStartAfterApproval: false,
    });
    await expect(target.service.start(target.runId, target.pdf.sha256, signal()))
      .resolves.toMatchObject({ bridgeState: "running" });
    expect(target.harness!.createCalls[0]?.resumePdf).toEqual(PDF_BYTES);
    expect(target.harness!.createCalls[0]?.resumeSource).toEqual(TAILORED_TEX_BYTES);
    expect(target.harness!.createCalls[0]?.transcriptPdf).toEqual(TRANSCRIPT_BYTES);
    expect(target.repository.getLatestApplicationSession(target.runId)?.resumeRevision).toBe(3);
  });

  test("automatically starts an approved QA retry with inherited resume files", async () => {
    const target = await createTarget({ skipReview: true, qaRetries: 2 });

    expect(await target.service.startNextAutomaticApplication(signal())).toBe(true);
    expect(await target.service.get(target.runId)).toMatchObject({ bridgeState: "running" });
    expect(target.repository.getLatestApplicationSession(target.runId)?.resumeRevision).toBe(3);
  });
  ...
  test("maps durable ninth-slot admission to the public busy error", async () => {
    const target = await createTarget({
      sessionIds: Array.from({ length: 9 }, () => crypto.randomUUID()),
    });
    ...
    for (let index = 2; index <= 8; index += 1) {
      ...
      await target.service.start(approved.run.id, approved.pdf.sha256, signal());
    }
    const waiting = await createApprovedRun(
      target.repository,
      target.database,
      target.artifacts,
      { id: "run-9" },
    ...
    await expect(
      target.service.start(waiting.run.id, waiting.pdf.sha256, signal()),
    ).rejects.toMatchObject({
      code: "APPLICATION_SESSION_BUSY",
      message: "Browser application sessions are currently unavailable",
      status: 409,
    });
    expect(target.repository.getLatestApplicationSession(waiting.run.id)).toBeNull();
  });
  ...
  test("opens the active application browser while review is pending", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    await target.service.start(target.runId, target.pdf.sha256, signal());

    await target.service.openBrowser(target.runId, signal());

    expect(target.harness!.openCalls).toEqual([FIRST_SESSION_ID]);
  });
  ...
  test("uploads explicit application defaults", async () => {
    const target = await createTarget({ autoSubmit: false });
    ...
    expect(target.harness!.createCalls).toHaveLength(1);
    expect(target.harness!.createCalls[0]).toMatchObject({
      autoSubmit: true,
      autoEnd: true,
    });
  });

  test("uploads the persisted auto-submit run mode by default", async () => {
  ...
  test("reports exact eligibility blockers without exposing private sources", async () => {
    ...
    await expect(
      ...
      code: "APPLICATION_SOURCE_UNAVAILABLE",
      status: 409,
      message: "Provide applicant-profile.md in your private candidate context before starting an application",
    });
  ...
  test("missing applicant profile blocks application start without a reservation or private path in the error", async () => {
    const { root } = await createProfileRoot();
    const target = await createTarget({ profileReader: () => readApplicantProfileMarkdown(root) });

    expect(await target.service.get(target.runId)).toMatchObject({
      state: "not_started",
      blockedReason: "profile_unavailable",
    });
    await expect(target.service.start(target.runId, target.pdf.sha256, signal())).rejects.toMatchObject({
      code: "APPLICATION_SOURCE_UNAVAILABLE",
      status: 409,
      message: "Provide applicant-profile.md in your private candidate context before starting an application",
    });
  ...
  test("transcript reader returns exact PDF bytes and rejects unsafe files", async () => {
    const root = await mkdtemp(join(tmpdir(), "application-transcript-reader-"));
    temporaryRoots.push(root);

    expect(await readApplicantTranscriptPdf(join(root, "missing.pdf"))).toBeUndefined();

    const transcriptPath = join(root, "transcript.pdf");
    await writeFile(transcriptPath, TRANSCRIPT_BYTES);
    expect(await readApplicantTranscriptPdf(transcriptPath)).toEqual(TRANSCRIPT_BYTES);

    const replacement = Uint8Array.from(TRANSCRIPT_BYTES);
    const finalByte = replacement.byteLength - 1;
    replacement[finalByte] = (replacement[finalByte] ?? 0) ^ 1;
    const probe = await open(transcriptPath, "r");
    const fileHandlePrototype = Object.getPrototypeOf(probe) as {
      read: (this: unknown, ...args: never[]) => Promise<unknown>;
    };
    await probe.close();
    const originalRead = fileHandlePrototype.read;
    let rewritten = false;
    fileHandlePrototype.read = async function (this: unknown, ...args: never[]) {
    ...
    try {
      await expect(readApplicantTranscriptPdf(transcriptPath)).rejects.toThrow(/changed/);
    } finally {
    ...
    await expect(readApplicantTranscriptPdf(invalidPath)).rejects.toThrow(/PDF/);

    const linkedPath = join(root, "linked.pdf");
    await symlink(transcriptPath, linkedPath);
    await expect(readApplicantTranscriptPdf(linkedPath)).rejects.toThrow(/symlink/);
  });
  ...
  test("persists terminal harness events without a client stream", async () => {
    const harness = new FakeHarness();
    harness.events = [{
      id: 1,
      event: "failed",
    ...
    const target = await createTarget({
      harness,
      onApplicationSessionReleased: () => released.resolve(),
    });

    expect(await target.service.start(target.runId, target.pdf.sha256, signal()))
      .toMatchObject({ bridgeState: "running" });
  ...
  test("keeps observing cleanup after the client event stream ends", async () => {
    ...
    await released.promise;
    expect(target.harness!.streamCalls).toContainEqual({
      sessionId: FIRST_SESSION_ID,
      lastEventId: 1,
    });
  ...
  test("stops cleanup observers without releasing the slot during service disposal", async () => {
    ...
    const observerSignal = target.harness!.streamSignals[0];
  ...
  test("preserves a human-navigation handoff after submission is claimed", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    await target.service.start(target.runId, target.pdf.sha256, signal());
    target.repository.claimApplicationSubmission(FIRST_SESSION_ID);
    ...
    target.harness!.events = [{
      ...
      session: {
        ...
        pendingAction: {
          type: "human_navigation",
          instruction: "Inspect the application page, then return control.",
        },
      },
      detail: { instruction: "Inspect the application page, then return control." },
    }];
    const events: ApplicationSessionEventDto[] = [];
    ...
    expect(events).toEqual([expect.objectContaining({
      ...
      session: expect.objectContaining({
        bridgeState: "awaiting_human_navigation",
        submissionPhase: "attempting",
        pendingAction: expect.objectContaining({ type: "human_navigation" }),
      }),
    })]);
    ...
    await target.service.command(target.runId, { type: "continue" }, signal());
    for (const command of [
      { type: "submit" },
      { type: "continue_without_additional_info" },
      { type: "steer", message: "Try submitting the application again." },
    ] as const) {
      await expect(target.service.command(target.runId, command, signal())).rejects.toMatchObject({
        code: "APPLICATION_COMMAND_CONFLICT",
        status: 409,
      });
    }
    expect(() => target.repository.claimApplicationSubmission(FIRST_SESSION_ID))
    ...
    await target.service.startNextAutomaticApplication(signal());
    await expect(target.service.command(target.runId, { type: "continue" }, signal()))
      .rejects.toMatchObject({ code: "APPLICATION_COMMAND_CONFLICT", status: 409 });
  });

  test("recovers a previously masked handoff from an unchanged harness snapshot", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    await target.service.start(target.runId, target.pdf.sha256, signal());
    target.repository.claimApplicationSubmission(FIRST_SESSION_ID);
    const masked = await target.service.get(target.runId);
    target.setNow(30_000);
    target.repository.recordApplicationSnapshot(target.runId, {
      generation: 1,
      sessionId: FIRST_SESSION_ID,
      bridgeState: "submitting",
      publicSnapshot: { ...masked, updatedAt: 20_000 },
      slotReleased: false,
    });
    target.harness!.snapshots.set(FIRST_SESSION_ID, {
      ...harnessSnapshot("awaiting_human_navigation"),
      updatedAt: 20_000,
      pendingAction: { type: "human_navigation", instruction: "Return control after inspecting the page." },
    });
  ...
  test("cancels an attempted submission without making it retryable", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    await target.service.start(target.runId, target.pdf.sha256, signal());
    target.repository.claimApplicationSubmission(FIRST_SESSION_ID);

    await target.service.command(target.runId, { type: "cancel" }, signal());
    target.harness!.events = [{
      id: 1,
      event: "submission_uncertain",
      session: { ...harnessSnapshot("submission_uncertain"), updatedAt: 10_200 },
      detail: {},
    }];
    const events: ApplicationSessionEventDto[] = [];
    for await (const item of await target.service.events(target.runId, undefined, signal())) {
    ...
    expect(events.at(-1)?.session).toMatchObject({
      bridgeState: "submission_uncertain", submissionPhase: "uncertain", pendingAction: null,
    });
    expect(await target.service.get(target.runId)).toMatchObject({
      bridgeState: "submission_uncertain", submissionPhase: "uncertain", pendingAction: null,
    });
  ...
  test("requires explicit reapply intent before restarting a submitted application", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    ...
    await expect(target.service.start(
      target.runId,
      target.pdf.sha256,
      signal(),
      { reapply: true },
  ...
  test("retries a closed submitted application session", async () => {
    const target = await createTarget();
    target.harness!.snapshotAfterCreate = harnessReviewSnapshot();
    await target.service.start(target.runId, target.pdf.sha256, signal());
    target.repository.claimApplicationSubmission(FIRST_SESSION_ID);
