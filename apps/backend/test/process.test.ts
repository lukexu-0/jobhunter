...
test.skipIf(process.platform !== "darwin")("uses attempt-local TeX cache paths on macOS", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pipeline-process-darwin-"));
  let seen: SpawnContract | undefined;
  try {
    await runTrustedProcess({
      command: "latexmk",
    ...
    expect(seen?.attemptRootFd).toBeUndefined();
    expect(seen?.env).toMatchObject({
      PATH: "/Library/TeX/texbin:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin",
      TMPDIR: ".tex-cache/tmp",
      TEXMFHOME: ".tex-cache/texmf-home",
      TEXMFVAR: ".tex-cache/texmf-var",
      VARTEXFONTS: ".tex-cache/fonts",
      MT_VARTEXFONTS: ".tex-cache/fonts",
    });
    expect(JSON.stringify(seen?.env)).not.toContain(cwd);
  } finally {
    await rm(cwd, { recursive: true, force: true });
