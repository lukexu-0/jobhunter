...
describe("SourceHandoffService", () => {
  ...
  test("accepts a canonical final URL through the private 4096-character bound", async () => {
  ...
  test("completes a cross-origin capture exactly once while retaining the submitted application URL", async () => {
    const source = "Senior Engineer\nBuild reliable TypeScript services with careful testing and ownership.";
    const target = fixture({
      completeResult: {
        finalUrl: "https://careers.example.test/redirected-role",
        source,
      },
      runCreate: async (request) => ({ ...runDto(), jobUrl: request.jobUrl }),
    });
    await target.service.create(REQUEST);

    const [completed, repeated] = await Promise.all([
      target.service.complete(HANDOFF_ID),
      target.service.complete(HANDOFF_ID),
    ]);
    expect(completed).toEqual(runDto());
    expect(repeated).toEqual(completed);
    await expect(target.service.complete(HANDOFF_ID)).resolves.toEqual(completed);
    expect(target.calls).toMatchObject({
      creates: 1,
      kicks: 1,
      captureDeletes: [HANDOFF_ID],
      capturedSources: [source],
    });
  });
