...
const testWithNativeTexCache = test.skipIf(process.platform !== "linux" && process.platform !== "darwin");
...
describe("trusted resume compiler", () => {
  test("rejects a synthetic glyph mapping preamble not matching the trusted production baseline", async () => {
    let spawned = false;
    const artifacts = new ArtifactStore(await root());
    const result = await compileResume({
      artifacts,
      address,
      tex: SYNTHETIC_RESUME,
      mode: "full",
      processBoundary: () => { spawned = true; return exited(); },
    });
    expect(result.ok).toBeFalse();
    if (!result.ok) expect(result.reason).toMatch(/forbidden TeX primitive/i);
    expect(spawned).toBeFalse();
  });

  test("rejects noncanonical glyph mapping inputs before spawning", async () => {
    const canonical = SYNTHETIC_RESUME;
  ...
  testWithNativeTexCache("uses a locked recipe, isolated policy, owner-only local TeX caches, and candidate/full timeouts", async () => {
    for (const mode of ["candidate", "full"] as const) {
      ...
      const boundary: ProcessBoundary = (contract) => {
        ...
        if (process.platform === "linux") {
          ...
          expect(contract.attemptRootFd).toBeUndefined();
        }
      ...
      const cacheRoot = process.platform === "linux"
        ? `/proc/${process.pid}/fd/${seen?.attemptRootFd}/.tex-cache`
        : ".tex-cache";
      ...
      if (process.platform === "linux") expect(() => fstatSync(seen!.attemptRootFd!)).toThrow();
    }
  });

  testWithNativeTexCache("always finalizes a bounded log and publishes no PDF on failure", async () => {
    const artifacts = new ArtifactStore(await root());
  ...
  testWithNativeTexCache("distinguishes repairable TeX diagnostics from terminal compiler failures", async () => {
    const cases = [
      ["! Undefined control sequence.\\n\\resumeIten", "repairable"],
      ["! File ended while scanning use of \\resumeItem.", "repairable"],
      [String.raw`./main.tex:153: LaTeX Error: Something's wrong--perhaps a missing \item.`, "repairable"],
  ...
  testWithNativeTexCache("publishes a capped immutable PDF only after successful compilation", async () => {
    const artifacts = new ArtifactStore(await root());
  ...
  testWithNativeTexCache("turns an oversized success PDF into a terminal failure with a finalized log", async () => {
    const artifacts = new ArtifactStore(await root());
    const result = await compileResume({
      artifacts,
      address,
