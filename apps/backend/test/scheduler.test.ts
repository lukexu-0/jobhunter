...
describe("concurrent worker scheduler", () => {
  test("runs exactly thirty processors concurrently, waits to start a thirty-first, and releases exact claims", async () => {
    const claims = Array.from({ length: 31 }, (_, index) => claim("run-" + (index + 1), index + 1));
    const queue = [...claims];
    ...
    let signalThirtyStarted!: () => void;
    const thirtyStarted = new Promise<void>((resolve) => { signalThirtyStarted = resolve; });
    let signalThirtyFirstStarted!: () => void;
    const thirtyFirstStarted = new Promise<void>((resolve) => { signalThirtyFirstStarted = resolve; });
    const started: string[] = [];
    const scheduler = new WorkerScheduler(repository, async (value) => {
      started.push(value.runId);
      if (started.length === 30) signalThirtyStarted();
      if (value.runId === "run-31") signalThirtyFirstStarted();
      await new Promise<void>((resolve) => { gates.set(value.runId, resolve); });
    }, { heartbeatMs: 1_000 });

    scheduler.kick();
    await thirtyStarted;
    expect(new Set(started)).toEqual(new Set(Array.from({ length: 30 }, (_, index) => "run-" + (index + 1))));
    expect(released).toEqual([]);

    gates.get("run-1")!();
    await thirtyFirstStarted;
    expect(new Set(started)).toEqual(new Set(Array.from({ length: 31 }, (_, index) => "run-" + (index + 1))));
    expect(released).toEqual([{ runId: claims[0]!.runId, token: claims[0]!.token }]);

    for (let index = 2; index <= 31; index += 1) gates.get("run-" + index)!();
    await scheduler.waitForIdle();
    expect(released).toHaveLength(31);
