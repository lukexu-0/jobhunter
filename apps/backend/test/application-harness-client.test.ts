...
describe("HttpApplicationHarnessClient", () => {
  ...
  test("maps declared Gmail OAuth unavailability from start and poll", async () => {
    const responses = [
      Response.json(
        { code: "gmail_oauth_unavailable", message: "Gmail connection is unavailable" },
        { status: 503 },
      ),
      Response.json(
        { code: "gmail_oauth_unavailable", message: "Gmail connection is unavailable" },
    ...
    await expect(client.createGmailAuthSession(signal)).rejects.toEqual(
      new ApplicationHarnessError("unavailable"),
    );
    await expect(client.getGmailAuthSession(GMAIL_SESSION_ID, signal)).rejects.toEqual(
      new ApplicationHarnessError("unavailable"),
    );
  });

  test("accepts the producer's full Google authorization URL length", async () => {
    const authorizationUrl = "https://accounts.google.com/o/oauth2/v2/auth?state=" + "x".repeat(4_096);
    const client = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
    ...
    await expect(client.createGmailAuthSession(new AbortController().signal)).resolves.toEqual({
      id: GMAIL_SESSION_ID,
      state: "pending",
      authorizationUrl,
      expiresAt: Date.parse("2026-09-17T12:00:00Z"),
    });
  });

  test("rejects authorization URLs outside the Google authorization origin", async () => {
    const client = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
      fetchImpl: async () => Response.json({
        id: GMAIL_SESSION_ID,
        state: "pending",
        authorization_url: "https://accounts.google.com.evil.test/o/oauth2/v2/auth",
        expires_at: "2026-09-17T12:00:00Z",
      }, { status: 201 }),
    });

    await expect(client.createGmailAuthSession(new AbortController().signal)).rejects.toEqual(
      new ApplicationHarnessError("invalid_response"),
    );
  });

  test("accepts valid model metadata and rejects mismatched identities", async () => {
    ...
    const client = new HttpApplicationHarnessClient({
      ...
      fetchImpl: async () => Response.json(rawSnapshot({
        model_provider: "google-antigravity", model, reasoning,
      })),
    });
    const snapshot = await client.get(SESSION_ID, new AbortController().signal);
    expect(snapshot.state).toBe("awaiting_additional_info");
    expect(snapshot.pendingAction?.type).toBe("additional_info");
    reasoning = "medium";
    await expect(client.get(SESSION_ID, new AbortController().signal)).rejects.toBeInstanceOf(ApplicationHarnessError);
    reasoning = "high";
    model = "gpt-5.6-sol";
    await expect(client.get(SESSION_ID, new AbortController().signal)).rejects.toBeInstanceOf(ApplicationHarnessError);
  });
  ...
  test("creates a caller-ID session with only the fixed multipart parts", async () => {
    ...
    expect(form.get("auto_end")).toBe("true");
  ...
  test("adds an optional transcript as a fixed PDF multipart part", async () => {
    ...
    const transcriptPdf = new TextEncoder().encode("%PDF-1.7\nofficial transcript");
    const injectedBytes = new Uint8Array(1_024);
    injectedBytes.set(transcriptPdf);
    Object.defineProperty(transcriptPdf, Symbol.iterator, {
      value: () => injectedBytes.values(),
    });
    ...
    await client.create({
      ...
      autoSubmit: false,
      autoEnd: false,
      resumePdf: new TextEncoder().encode("%PDF-1.7\napproved resume"),
      resumeSource: new TextEncoder().encode("\\documentclass{article}"),
      transcriptPdf,
    }, new AbortController().signal);

    const transcript = capturedForm?.get("transcript");
    expect(transcript).toBeInstanceOf(File);
    if (!(transcript instanceof File)) throw new Error("Expected transcript file part");
    expect({
      name: transcript.name,
      type: transcript.type,
      bytes: new Uint8Array(await transcript.arrayBuffer()),
    }).toEqual({
      name: "Applicant_Transcript.pdf",
      type: "application/pdf",
      bytes: transcriptPdf,
    });
  });
  ...
  test("opens the exact active browser session with a bodyless request", async () => {
    ...
    await client.open(SESSION_ID, requestSignal);

    expect(call?.url).toBe(`${ORIGIN}/v1/sessions/${SESSION_ID}/browser/open`);
  ...
  test("accepts uppercase UUIDs when addressing a session", async () => {
    const sessionId = SESSION_ID.toUpperCase();
    ...
    await client.open(sessionId, new AbortController().signal);

    expect(requestedUrl).toBe(`${ORIGIN}/v1/sessions/${sessionId}/browser/open`);
  });
  ...
  test("validates session command text as Unicode scalars", async () => {
    ...
    await client.command(SESSION_ID, { type: "revise", context: maximumRevision }, signal);
    await client.command(SESSION_ID, { type: "revise", context: "\uFEFF" }, signal);
    ...
    expect(bodies).toEqual([
      { type: "revise", context: maximumRevision },
      { type: "revise", context: "\uFEFF" },
    ]);
  });
  ...
  test("maps only bounded stable harness and network failures", async () => {
    ...
    const responseCases = [
      ...
      {
        response: Response.json(
          { code: "session_not_found", message: "Session not found", session_id: null },
          { status: 404 },
        ),
      ...
      },
      {
        ...
        invoke: (client: HttpApplicationHarnessClient) =>
          client.suggestions(SESSION_ID, "preferred_name", new AbortController().signal),
      },
      {
        response: Response.json(
  ...
  test("sends only canonical HTTPS source-capture create URLs through the 2048-character bound", async () => {
    let fetchCalls = 0;
    const client = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
    ...
    for (const jobUrl of [
      ...
      await expect(client.createSourceCapture({
        ...
        new SourceCaptureHarnessError("invalid_request"),
      );
    }
    expect(fetchCalls).toBe(1);
  });

  test("accepts exactly 20,000 captured lines when the source ends in a newline", async () => {
    const source = "x\n".repeat(20_000);
    const client = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
    ...
    const overflow = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
      fetchImpl: async () => Response.json({
        capture_id: SESSION_ID,
    ...
    await expect(overflow.completeSourceCapture(
      SESSION_ID,
      new AbortController().signal,
  ...
  test("mirrors model credentials and selection over authenticated strict requests", async () => {
    const calls: Array<{ url: string; init: RequestInit | undefined }> = [];
    const client = new HttpApplicationHarnessClient({
      origin: ORIGIN,
      token: TOKEN,
    ...
    await client.setModelCredential("openai-codex", credential, signal);
    await client.deleteModelCredential("openai-codex", signal);
    await client.setApplicationModel("gemini-3.8-flash", signal);
    ...
    expect(calls.map(({ url, init }) => ({
      ...
      {
        url: ORIGIN + "/v1/model-credentials/openai-codex",
        method: "PUT",
        authorization: "Bearer test-token-0123456789abcdef-0123456789",
      ...
      {
        url: ORIGIN + "/v1/application-model",
        method: "PUT",
        authorization: "Bearer test-token-0123456789abcdef-0123456789",
        contentType: "application/json",
