...
function service(overrides: Partial<RunRouteService> = {}) {
  ...
  const value: RunRouteService & { kickCount(): number } = {
    ...
    deleteRun: async () => {},
    retryRun: async () => run,
    retailorRun: async () => ({ ...run, revision: 1, status: "analyzing" }),
...
describe("run HTTP routes", () => {
  ...
  test("returns human verification without kicking a run", async () => {
    const code = "JOB_HUMAN_VERIFICATION_REQUIRED";
    const target = service({
      createRun: async () => {
  ...
  test("re-tailors the exact reviewed PDF and rejects malformed requests", async () => {
    let received: { id: string; expectedPdfSha256: string } | undefined;
    const target = service({
      retailorRun: async (id, expectedPdfSha256) => {
        received = { id, expectedPdfSha256 };
        return { ...run, status: "analyzing", revision: 1 };
      },
    });

    const response = await request(
      target,
      "/v1/runs/run-1/retailor",
      post({ expectedPdfSha256: PDF_HASH }),
    );

    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ status: "analyzing", revision: 1 });
    expect(received).toEqual({ id: "run-1", expectedPdfSha256: PDF_HASH });
    expect(target.kickCount()).toBe(1);

    const invalid = await request(
      target,
      "/v1/runs/run-1/retailor",
      post({ expectedPdfSha256: PDF_HASH, extra: true }),
    );
  ...
  test("returns stable HTTP 410 responses for pruned downloads and lifecycle commands", async () => {
    ...
    const target = service({
      ...
      retailorRun: async () => { throw pruned; },
    ...
    const commands: readonly [string, unknown][] = [
      ["/v1/runs/run-1/retry", {}],
      ["/v1/runs/run-1/retailor", { expectedPdfSha256: PDF_HASH }],
...
describe("application session HTTP routes", () => {
  test("gets a validated view and forwards explicit application options", async () => {
    let received:
      | {
          runId: string;
          expectedApprovedPdfSha256: string;
          signal: AbortSignal;
          options: { autoSubmit?: boolean; autoEnd?: boolean; reapply?: boolean };
        }
      | undefined;
    const target = applicationService({
      start: async (runId, expectedApprovedPdfSha256, signal, options) => {
        received = { runId, expectedApprovedPdfSha256, signal, options };
    ...
    expect(received).toEqual({
      ...
      options: { autoSubmit: true, autoEnd: true, reapply: true },
    });
  });

  test("opens the application browser through a bodyless run action", async () => {
    let received: { runId: string; signal: AbortSignal } | undefined;
    const target = applicationService({
      openBrowser: async (runId, requestSignal) => {
        received = { runId, signal: requestSignal };
      },
    });
    const path = "/v1/runs/run-1/application/browser/open";

    const response = await applicationRequest(target, path, {
      method: "POST",
      headers: { origin: ORIGIN },
    });
    ...
    const rejected = await applicationRequest(target, path, post({}));
    expect(rejected.status).toBe(400);
  });
  ...
  test("rejects malformed requests through the public Origin and JSON boundary", async () => {
    ...
    for (const command of [
      ...
      { type: "approve_origin", origin: "https://example.test" },
