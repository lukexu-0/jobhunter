...
function createRepositoryFixture() {
  ...
  fixtures.push(root);
  const manifestRelativePath = "apps/backend/context-sources.json";
  ...
  const baselinePath = join(root, BASELINE_CONTEXT_SOURCE_PATH);
  mkdirSync(dirname(baselinePath), { recursive: true });
  writeFileSync(baselinePath, "Synthetic candidate resume with verifiable qualifications.");
  return {
    root,
...
describe("ContextApplicationService", () => {
  ...
  test("creates a fresh snapshot containing the indexed baseline hash", () => {
    ...
    try {
      ...
      expect(Object.keys(snapshot.sourceHashes)).toHaveLength(1);
  ...
  test("loads the canonical baseline content and matching hash without exposing resolved paths or changing sources", () => {
    ...
    const before = digest(baselineBytes);
    const database = openContextDatabase(":memory:");
    const service = createContextApplicationService({ database, loadedManifest });
    try {
      service.syncContext();
      ...
      expect(digest(readFileSync(join(root, BASELINE_CONTEXT_SOURCE_PATH)))).toBe(before);
    } finally {
      service.close();
      database.close();
    }
  ...
  test("refuses snapshot and stage loads after source drift", () => {
    const { root, loadedManifest } = createRepositoryFixture();
    const database = openContextDatabase(":memory:");
    const service = createContextApplicationService({ database, loadedManifest });
    try {
      service.syncContext();
      const changedSource = loadedManifest.manifest.sources[0]!;
