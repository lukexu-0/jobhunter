...
import { SYNTHETIC_CONTEXT_SOURCES, writeSyntheticContextSources } from "./private-context.fixture.ts";
...
function fixture(options: FixtureOptions = {}): Fixture {
  ...
  const manifest = JSON.parse(readFileSync(join(REPOSITORY_ROOT, manifestRelative), "utf8"));
  for (const [relativePath] of Object.entries(SYNTHETIC_CONTEXT_SOURCES).slice(1)) {
    manifest.sources.push({
      id: relativePath.split("/").at(-1)!.replace(/\.md$/, ""),
      relativePath,
      kind: "authoritative-markdown",
      entityId: relativePath,
      displayName: "Synthetic context source",
      baselineEntityIds: [],
    });
  }
  writeFileSync(join(root, manifestRelative), JSON.stringify(manifest));
  writeSyntheticContextSources(root);
...
test("exposes only verified current-revision QA check IDs before review", async () => {
  const target = fixture();
  const run = await target.service.createRun(urlRunRequest());
  const claim = target.repository.acquire()!;
  transition(target.repository, claim, ["analyzing", "tailoring", "compiling", "deterministic_qa"]);
  const attempt = target.repository.startAttempt(claim, "deterministic_qa");
  const root = await target.artifacts.createAttempt({ run: run.queueSequence, revision: "1", stage: "deterministic_qa", attempt: 1 });
  const stored = await target.artifacts.write(join(root, "deterministic-qa.json"), JSON.stringify({
    pass: false,
    checks: [
      { id: "one-page", status: "pass", detail: "private candidate data" },
      { id: "text-output", status: "fail", detail: "private tool stderr /private/path" },
      { id: "required-headings", status: "fail", detail: "private candidate data" },
    ],
    warnings: ["private log"],
  }), 4096);
  const artifact = target.repository.finalizeArtifact(claim, {
    attemptId: attempt.id, stage: "deterministic_qa", kind: "deterministic-qa",
    sha256: stored.sha256, path: stored.path, byteSize: stored.bytes,
  });
  target.repository.finishAttempt(claim, attempt.id, "failed");
  target.repository.transition(claim, "failed", { failedStage: "deterministic_qa" });
  target.repository.release(claim);
  const dto = RunDtoSchema.parse(await target.service.getRun(run.id));
  expect(dto).toMatchObject({ failureChecks: ["text-output", "required-headings"] });
  expect(JSON.stringify(dto)).not.toContain("private");
  await expect(target.service.getArtifact(run.id, artifact.id)).resolves.toBeUndefined();

  const retainedReport = readFileSync(stored.path);
  writeFileSync(stored.path, "{}");
  expect(await target.service.getRun(run.id)).not.toHaveProperty("failureChecks");
  writeFileSync(stored.path, retainedReport);
  await target.service.retryRun(run.id);
  const retryClaim = target.repository.acquire()!;
  target.repository.transition(retryClaim, "failed", { failedStage: "deterministic_qa" });
  target.repository.release(retryClaim);
  expect(await target.service.getRun(run.id)).not.toHaveProperty("failureChecks");
});
...
describe("RunApplicationService", () => {
  ...
  test("synchronizes stale context once and creates the run from the refreshed snapshot", async () => {
    ...
    const changedPath = join(target.root, BASELINE_CONTEXT_SOURCE_PATH);
  ...
  test("missing resume baseline rejects run creation with setup guidance and no persisted run", async () => {
    const target = fixture();
    unlinkSync(join(target.root, BASELINE_CONTEXT_SOURCE_PATH));
    const route = createApiHandler({ webOrigin: ORIGIN, route: createRunRoutes(target.service) });
    ...
    expect(await response.json()).toEqual({ error: {
      code: "CONTEXT_SOURCES_MISSING",
      message: "Provide a resume baseline in your private candidate context before continuing",
    } });
    expectNoPersistence(target);
  });
  ...
  test("exposes durable run modes and canonical job URLs on created, listed, and retrieved DTOs", async () => {
    ...
    delete legacyDto.isApplicationSessionOpen;
    expect(RunDtoSchema.parse(legacyDto)).not.toHaveProperty("isApplying");
    expect(RunDtoSchema.parse(legacyDto)).not.toHaveProperty("isApplicationSessionOpen");
  });

  test("projects application activity independently of the editable lifecycle status", async () => {
    const target = fixture();
    ...
    expect(approved).toMatchObject({ isApplying: false, isApplicationSessionOpen: false });
    ...
    const listed = await target.service.listRuns();
    expect(listed.find(({ id }) => id === applying.id)).toMatchObject({ isApplying: true, isApplicationSessionOpen: true });
    expect(listed.find(({ id }) => id === ordinary.id)).toMatchObject({ isApplying: false, isApplicationSessionOpen: false });
    ...
    ).toMatchObject({ isApplying: false, isApplicationSessionOpen: false });
  ...
  test("projects application failures separately from lifecycle status and clears them on retry", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    await target.service.approveRun(run.id, pdf.sha256, false);
    const firstSession = "93939393-9393-4393-8393-939393939393";
    const retrySession = "94949494-9494-4494-8494-949494949494";
    target.repository.reserveApplicationSession(run.id, null, firstSession, pdf.sha256);
    expect(await target.service.getRun(run.id)).not.toHaveProperty("applicationFailureGeneration");
    target.repository.recordApplicationSnapshot(run.id, {
      slotReleased: true, generation: 1, sessionId: firstSession,
      bridgeState: "failed", publicSnapshot: { state: "failed" },
    });
    expect(await target.service.getRun(run.id)).toMatchObject({
      status: "approved", applicationStatus: "pending", isApplying: false,
      applicationFailureGeneration: 1,
    });
    expect((await target.service.listRuns()).find(({ id }) => id === run.id))
      .toMatchObject({ applicationFailureGeneration: 1 });

    target.repository.reserveApplicationSession(run.id, firstSession, retrySession, pdf.sha256);
    expect(await target.service.getRun(run.id)).not.toHaveProperty("applicationFailureGeneration");
    const retrying = (await target.service.listRuns()).find(({ id }) => id === run.id)!;
    expect(retrying.isApplying).toBe(true);
    expect(retrying).not.toHaveProperty("applicationFailureGeneration");
    target.repository.recordApplicationSnapshot(run.id, {
      slotReleased: true, generation: 2, sessionId: retrySession,
      bridgeState: "failed", publicSnapshot: { state: "failed" },
    });
    const failedRetry = (await target.service.listRuns()).find(({ id }) => id === run.id)!;
    expect(RunDtoSchema.parse(failedRetry)).toMatchObject({ applicationFailureGeneration: 2 });
  });
  ...
  test("retry endpoint refreshes a changed resume source before starting a new analysis", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const originalSources = target.repository.getSourceSnapshot(run.id)!;
    const claim = target.repository.acquire()!;
    transition(target.repository, claim, ["analyzing", "tailoring", "compiling"]);
    target.repository.transition(claim, "failed", { failedStage: "compiling" });
    target.repository.release(claim);
    const baselinePath = join(target.root, BASELINE_CONTEXT_SOURCE_PATH);
    writeFileSync(baselinePath, `${readFileSync(baselinePath, "utf8")}\n% Updated resume source for retry.\n`);
    const route = createApiHandler({ webOrigin: ORIGIN, route: createRunRoutes(target.service) });

    const response = await route(new Request(`http://127.0.0.1:3457/v1/runs/${run.id}/retry`, post({})));

    expect(response.status).toBe(200);
    const retried = RunDtoSchema.parse(await response.json());
    expect(retried).toMatchObject({ id: run.id, status: "analyzing", revision: 2, origin: "initial" });
    expect(retried.timeline.at(-1)).toMatchObject({ type: "run.retried", status: "analyzing" });
    expect(target.repository.getSourceSnapshot(run.id)?.baselineSha256).not.toBe(originalSources.baselineSha256);
    expect(target.repository.getSourceSnapshot(run.id, 1)?.baselineSha256).toBe(originalSources.baselineSha256);
  });
  ...
  test("enforces review/hash invariants and preserves exact inert edit comments", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    ...
    const changedPath = join(target.root, target.loaded.manifest.sources[1]!.relativePath);
    writeFileSync(changedPath, `${readFileSync(changedPath, "utf8")}\nChanged authoritative source.\n`);
    await expect(target.service.retryRun(run.id)).rejects.toMatchObject({ code: "RUN_CONFLICT", status: 409 });
  });

  test("approves an immutable review revision after authoritative resume sources change", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    const changedPath = join(target.root, target.loaded.manifest.sources[1]!.relativePath);
    ...
    const approved = await target.service.approveRun(run.id, pdf.sha256, false);

    expect(approved).toMatchObject({
      status: "approved",
      revision: 1,
      currentPdfSha256: pdf.sha256,
    });
    expect(target.syncs.count).toBe(0);
  });
  test("re-tailors a pending approved run from changed authoritative resume sources", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    await target.service.approveRun(run.id, pdf.sha256, false);
    const originalSnapshot = target.repository.getSourceSnapshot(run.id)!;
    const changedPath = join(target.root, target.loaded.manifest.sources[1]!.relativePath);
    writeFileSync(changedPath, readFileSync(changedPath, "utf8") + "\nChanged authoritative source.\n");

    const refreshed = await target.service.retailorRun(run.id, pdf.sha256);
    ...
    expect(refreshed.currentPdfSha256).toBeUndefined();
    expect(target.repository.getSourceSnapshot(run.id)?.sourceHashes)
      .not.toEqual(originalSnapshot.sourceHashes);
    expect(target.repository.getArtifact(run.id, "compiled-pdf")).toBeNull();
    expect(target.syncs.count).toBe(1);
    expect(target.repository.getArtifact(run.id, "compiled-pdf", 1)?.sha256).toBe(pdf.sha256);
  });

  test("re-tailors only after an application session closes before submission", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    await target.service.approveRun(run.id, pdf.sha256, false);
    const sessionId = "11111111-1111-4111-8111-111111111111";
    const session = target.repository.reserveApplicationSession(run.id, null, sessionId, pdf.sha256);
    const changedPath = join(target.root, target.loaded.manifest.sources[1]!.relativePath);
    writeFileSync(changedPath, readFileSync(changedPath, "utf8") + "\nChanged authoritative source.\n");

    await expect(target.service.retailorRun(run.id, pdf.sha256)).rejects.toMatchObject({
      code: "RUN_CONFLICT",
      status: 409,
    });
    expect(await target.service.getRun(run.id)).toMatchObject({
    ...
    target.repository.abandonApplicationSessionReservation(
      run.id,
      session.generation,
      sessionId,
    );
    await expect(target.service.retailorRun(run.id, pdf.sha256)).resolves.toMatchObject({
      status: "analyzing",
      applicationStatus: "pending",
      revision: 2,
    });
  ...
  test("requires a pending application before re-tailoring", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    await target.service.updateApplicationStatus(run.id, "did_not_apply");
    const changedPath = join(target.root, target.loaded.manifest.sources[1]!.relativePath);
    writeFileSync(changedPath, readFileSync(changedPath, "utf8") + "\nChanged authoritative source.\n");
    ...
    expect(await target.service.getRun(run.id)).toMatchObject({
      status: "review",
      applicationStatus: "did_not_apply",
      revision: 1,
    });
  });
  test("honors an explicit pending re-tailor when sources are already current", async () => {
    const target = fixture();
    const run = await target.service.createRun(urlRunRequest());
    const pdf = await finalizeReviewPdf(target, run.id);
    ...
    expect(
      refreshed.timeline.find((event) => event.type === "run.retailored")?.detail,
    ).toMatchObject({ sourcesChanged: false });
  });
  ...
  test("maps every artifact-dependent command on a reserved run to HTTP 410", async () => {
    ...
    await expect(target.service.retailorRun(reviews[1]!.id, reviews[1]!.pdfSha256)).rejects.toMatchObject(expected);
  ...
  test("serves public resume and JSON artifacts with their public media metadata", async () => {
    ...
    expect(response?.headers.get("content-disposition")).toBe('inline; filename="Applicant_Resume.pdf"');
    ...
    expect(tex?.headers.get("content-disposition")).toBe('attachment; filename="Applicant_Resume.tex"');
...
describe("public run and context routes", () => {
  ...
  test("reports freshness, synchronizes stale context, bounds status arrays and never caches", async () => {
    const target = fixture();
    const changedPath = join(target.root, target.loaded.manifest.sources[2]!.relativePath);
    writeFileSync(changedPath, `${readFileSync(changedPath, "utf8")}\nDrift.\n`);
