...
export const AuthProviderSchema = z.enum(["openai-codex", "google-antigravity", "gmail"]);
export type AuthProvider = z.infer<typeof AuthProviderSchema>;

export const ModelAuthProviderSchema = z.enum(["openai-codex", "google-antigravity"]);
export type ModelAuthProvider = z.infer<typeof ModelAuthProviderSchema>;

export const ApplicationModelIdSchema = z.enum(["gpt-5.6-sol", "gemini-3.8-flash"]);
export type ApplicationModelId = z.infer<typeof ApplicationModelIdSchema>;
export const ApplicationModelSelectionSchema = z.object({ model: ApplicationModelIdSchema }).strict();
...
export const AuthStatusResponseSchema = z
  .object({
    providers: z.tuple([
      ...
      AuthProviderStatusSchema.extend({ provider: z.literal("google-antigravity") }),
      AuthProviderStatusSchema.extend({ provider: z.literal("gmail") }),
    ]),
  })
  .strict();
...
const ApplicationAnswerTextSchema = z.string()
...
export const DeterministicQaCheckIdSchema = z.enum([
  "pdfinfo-output", "unencrypted", "one-page", "page-count-consistency",
  "letter-size", "text-output", "required-headings", "font-output",
  "embedded-fonts", "word-bounds",
]);
export type DeterministicQaCheckId = z.infer<typeof DeterministicQaCheckIdSchema>;
...
export const RunDtoSchema = z
  .object({
    ...
    isApplicationSessionOpen: z.boolean().optional(),
    applicationFailureGeneration: z.number().int().positive().optional(),
    ...
    failureChecks: z.array(DeterministicQaCheckIdSchema).max(10).optional(),
...
export const ApplicationSessionErrorSchema = z.discriminatedUnion("code", [
  ...
  z.object({
    code: z.literal("usage_exhausted"),
    message: z.literal(APPLICATION_SESSION_ERROR_MESSAGES.usage_exhausted),
  }).strict(),
  z.object({
...
export const EditRunRequestSchema = z
  ...
  .strict();
export const RetailorRunRequestSchema = z
