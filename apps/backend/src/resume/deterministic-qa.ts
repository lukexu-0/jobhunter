...
import { parsePdfTextTsv } from "./pdf-text.ts";
import type { DeterministicQaCheckId } from "../contracts/index.ts";
...
function parseTextOutput(value: string): TextOutput | null {
  const pages = parsePdfTextTsv(value);
  const first = pages?.[0];
  if (!pages || !first || first.words.length === 0) return null;
  return {
    pageCount: pages.length,
    pageTexts: pages.map((page) => normalizeVisibleText(page.words.map(({ text }) => text).join(" "))),
    pageWidth: first.width,
    pageHeight: first.height,
    words: first.words,
    overflowLineCount: pages.reduce((count, page, index) => count + (index === 0 ? 0 : page.visibleLineCount), 0),
  };
}
...
export async function runDeterministicPdfQa(options: DeterministicQaOptions): Promise<DeterministicQaReport> {
  ...
  const [infoTool, textTool, fontTool] = await Promise.all([
    ...
    runTextTool("pdftotext", ["-tsv", "-enc", "UTF-8", pdfPath, "-"], options),
