...
interface BodySplit { readonly bodies: Readonly<Partial<Record<ResumeSection, string>>>; readonly immutableSkeleton: string; }
...
function splitBodies(source: string): BodySplit {
  ...
  const bodies: Partial<Record<ResumeSection, string>> = {};
  let immutableSkeleton = "";
  let cursor = 0;
  for (const { section, marker } of definitions) {
    const headingStart = source.indexOf(marker);
    if (headingStart < 0 && section === "competitions-other") continue;
    ...
    if (headingStart < cursor) throw new ResumeValidationError("editable sections were reordered");
    ...
    // Only the optional heading may disappear; all other surrounding source stays immutable.
    immutableSkeleton += source.slice(cursor, section === "competitions-other" ? headingStart : bodyStart);
    if (section !== "competitions-other") immutableSkeleton += `\u0000${section}\u0000`;
    bodies[section] = source.slice(bodyStart, bodyEnd);
    cursor = bodyEnd;
  }
