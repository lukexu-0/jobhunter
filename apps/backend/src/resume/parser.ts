...
type ResumeRegions = Record<Exclude<ResumeSection, "competitions-other">, ResumeRegion> & Partial<Record<"competitions-other", ResumeRegion>>;
export interface ParsedBaselineResume {
  readonly source: string;
  readonly sha256: string;
  readonly regions: Readonly<ResumeRegions>;
...
function findSection(source: string, heading: string, section: ResumeSection): ResumeRegion | undefined {
...
export function parseBaselineResume(source: string): ParsedBaselineResume {
  ...
  const experience = findSection(source, "Experience", "experience");
  const projects = findSection(source, "Projects", "projects");
  const competitionsOther = findSection(source, "Competitions \\& Other", "competitions-other");
  const technicalSkills = findSection(source, "Technical Skills", "technical-skills");
  if (!experience) throw new Error("missing required section Experience");
  if (!projects) throw new Error("missing required section Projects");
  if (!technicalSkills) throw new Error("missing required section Technical Skills");
  const regions: ResumeRegions = { experience, projects, "technical-skills": technicalSkills };
  const entities = [...parseEntities(experience), ...parseEntities(projects)];
  if (competitionsOther) {
    regions["competitions-other"] = competitionsOther;
    entities.push(...parseEntities(competitionsOther));
  }
