...
import { findChromeExecutable } from "./chrome.ts";
export type DoctorCheckStatus = "ok" | "warning" | "error";

export interface DoctorCheck {
  id: "bun" | "google-chrome" | "systemd-run" | "systemctl"
    | "browser-source" | "context" | "auth-openai" | "model-openai" | "model-luna"
...
async function toolCheck(
  ...
  processProbe: DoctorProcessProbe,
  platform = process.platform,
): Promise<DoctorCheck> {
  try {
    const executablePath = id === "google-chrome"
      ? findChromeExecutable((command) => processProbe.findExecutable(command), platform)
      : processProbe.findExecutable(id);
...
export async function runDoctor(dependencies: DoctorDependencies, options: DoctorOptions = {}): Promise<DoctorReport> {
  ...
  const remainingTools = await Promise.all(TOOL_IDS.slice(1)
    .filter((id) => platform === "linux" || (id !== "systemd-run" && id !== "systemctl"))
    .map((id) => toolCheck(id, processProbe, platform)));
  const checks: DoctorCheck[] = [
    bun, context, authOpenai, modelOpenai, modelLuna, ...remainingTools,
  ];
  if (platform === "darwin") {
    checks.push(check("browser-source", "warning", "native_browser_isolation",
      "JavaScript-only imports use native headless Chrome with its sandbox and a validated HTTPS proxy. macOS does not add Linux OS-level filesystem, network, memory, CPU or process-count limits. Visible human verification still requires the browser harness."));
  } else if (platform !== "linux") {
    checks.push(check("browser-source", "error", "unsupported_platform"));
  }
