...
export type ApplicationSubmissionPhase =
  ...
  | "submitted"
  | "uncertain";
export const APPLICATION_BROWSER_SESSION_CAPACITY = 8;
...
export class SourceDriftError extends RepositoryConflictError {
  ...
  constructor(message = "authoritative resume sources changed; re-tailor this completed run or retry a failed run") {
    super(message);
...
export class PipelineRepository {
  ...
  getApplicationModel(fallback: ApplicationModelId): ApplicationModelId {
    const row = this.#db.query<{ model: string }, []>(
      "SELECT model FROM application_model_selection WHERE id = 1",
    ).get();
    return row ? ApplicationModelIdSchema.parse(row.model) : ApplicationModelIdSchema.parse(fallback);
  }

  setApplicationModel(model: ApplicationModelId): ApplicationModelId {
    const selected = ApplicationModelIdSchema.parse(model);
    return this.#immediate(() => {
      this.#db.query(`
        INSERT INTO application_model_selection(id, model) VALUES (1, ?)
  ...
  #validateSnapshot(snapshot: RunSourceSnapshotInput): Readonly<Record<string, string>> {
    ...
    // Context manifests contain 1–20 uniquely identified sources, including resume-baseline.
    if (hashes.length < 1 || hashes.length > 20 || hashes.some(([id, hash]) =>
      id.length > 80 || !/^[a-z][a-z0-9-]*$/.test(id) || !/^[a-f0-9]{64}$/.test(hash)
    )) {
      throw new Error("source snapshot must contain 1 to 20 valid source IDs and lowercase SHA-256 hashes");
    }
    if (!Object.hasOwn(snapshot.sourceHashes, "resume-baseline") || snapshot.sourceHashes["resume-baseline"] !== snapshot.baselineSha256) {
      throw new Error("source snapshot baseline hash must match resume-baseline");
    }
  ...
  createQueuedRun(
    ...
    return this.#immediate(() => {
      ...
      this.#db.query("INSERT INTO run_source_snapshots(run_id,revision,manifest_sha256,baseline_sha256,source_hashes_json,created_at) VALUES (?,1,?,?,?,?)")
      ...
      this.#artifacts.insertInput(id, input, artifactId, now);
  ...
  listApplicationSessionStates(
    runIds: readonly string[],
  ): ReadonlyMap<string, Pick<PublicApplicationSession, "bridgeState" | "generation" | "slotReleased">> {
    const states = new Map<string, Pick<PublicApplicationSession, "bridgeState" | "generation" | "slotReleased">>();
    if (runIds.length === 0) return states;
    const placeholders = runIds.map(() => "?").join(",");
    const latest = this.#db.query<{
      runId: string;
      bridgeState: ApplicationSessionBridgeState;
      generation: number;
      slotReleased: 0 | 1;
    }, string[]>(`
      SELECT sessions.run_id AS runId, sessions.bridge_state AS bridgeState, sessions.generation,
             sessions.slot_released AS slotReleased
    ...
    for (const row of latest) {
      states.set(row.runId, {
        bridgeState: row.bridgeState,
        generation: row.generation,
        slotReleased: row.slotReleased === 1,
      });
    }
    return states;
  }
  ...
  reserveApplicationSession(
    ...
    options: Readonly<{
      ...
      allowTerminalApplicationRetry?: boolean;
    }> = {},
    ...
    return this.#immediate(() => {
      ...
      const reapplyingSubmitted = options.allowSubmittedApplication === true
        && run.application_status === "applied"
        && latest?.submission_phase === "submitted";
      const retryingTerminalApplication = options.allowTerminalApplicationRetry === true
        && latest !== null
        && TERMINAL_APPLICATION_SESSION_STATES[latest.bridge_state] === true;
      if (
        (
    ...
    const session = this.#immediate(() => {
      ...
      const bridgeMatchesSubmissionPhase =
        ...
        || (
          ...
          && (
            input.bridgeState === "submitting"
            || input.bridgeState === "awaiting_human_navigation"
          )
        )
        || (
          current.submission_phase === "submitted"
  ...
  attachSourceSnapshot(runId: string, snapshot: RunSourceSnapshotInput): PublicRunSourceSnapshot {
    ...
    return this.#immediate(() => {
      ...
      this.#db.query("INSERT INTO run_source_snapshots(run_id,revision,manifest_sha256,baseline_sha256,source_hashes_json,created_at) VALUES (?,?,?,?,?,?)")
        .run(runId, run.current_revision, snapshot.manifestSha256, snapshot.baselineSha256, sourceHashesJson, now);
  ...
  getSourceSnapshot(runId: string, revision?: number): PublicRunSourceSnapshot | null {
    const row = this.#db.query<{
      ...
      SELECT s.manifest_sha256,s.baseline_sha256,s.source_hashes_json,s.created_at
      FROM run_source_snapshots s JOIN runs r ON r.id=s.run_id
      WHERE s.run_id=? AND s.revision<=COALESCE(?,r.current_revision)
      ORDER BY s.revision DESC LIMIT 1
    `).get(runId, revision ?? null);
    return row ? {
  ...
  #matchesSourceSnapshot(runId: string, current: RunSourceSnapshotInput, currentHashesJson: string): boolean {
    ...
    return expected !== null
      ...
      && JSON.stringify(expected.sourceHashes) === currentHashesJson;
  }

  #assertSourceSnapshot(runId: string, current: RunSourceSnapshotInput): void {
    if (!this.#matchesSourceSnapshot(runId, current, JSON.stringify(this.#validateSnapshot(current)))) {
  ...
  listRuns(limit = 10_000): PublicRun[] {
  ...
  resolveRevisionOrigin(
    ...
    while (revision?.origin === "retry") {
      ...
      if (revision.retry_stage === "analyzing") return "initial";
  ...
  finishAttempt(
    ...
    return this.#immediate(() => {
      ...
      for (const artifact of artifacts) {
        this.#publishArtifact(attempt, artifact, now);
      }
  ...
  finalizeArtifact(claim: Pick<RunClaim, "runId" | "token">, input: { id?: string; attemptId: string; stage: string; kind: string; sha256: string; path: string; byteSize: number; sourceArtifactId?: string }): PublicArtifact {
    return this.#immediate(() => {
      ...
      const id = this.#publishArtifact(attempt, input, now, input.id);
      const artifact = this.#artifacts.getById(run.id, id);
      if (!artifact) throw new RepositoryConflictError("artifact not found");
      return artifact;
    });
  }

  #publishArtifact(attempt: AttemptRow, input: AttemptArtifactInput, now: number, id = this.#idFactory()): string {
    try {
      this.#artifacts.publish(attempt, input, id, now);
    } catch (error) {
    ...
    this.#event(attempt.run_id, attempt.revision, "artifact.finalized", {
      artifactId: id,
  ...
  getArtifactById(runId: string, artifactId: string): PublicArtifact | null {
    return this.#artifacts.getById(runId, artifactId);
  }

  getArtifact(runId: string, kind: string, revision?: number): PublicArtifact | null {
    const run = this.#run(runId);
    return this.#artifacts.resolve(runId, kind, revision ?? run.current_revision);
  }

  listArtifacts(runId: string, revision?: number): PublicArtifact[] {
    const run = this.#run(runId);
    return this.#artifacts.list(runId, revision ?? run.current_revision);
  }

  listResolvedArtifacts(runId: string, revision?: number): PublicArtifact[] {
    const run = this.#run(runId);
    return this.#artifacts.listResolved(runId, revision ?? run.current_revision);
  }
  ...
  getEditRequest(runId: string, targetRevision?: number): PublicEditRequest | null {
    ...
    while (!seen.has(selected)) {
      ...
      if (revision?.origin !== "retry" || revision.retry_stage === "analyzing" || revision.source_revision === null) break;
  ...
  retry(runId: string, currentSources?: RunSourceSnapshotInput): PublicRun {
    return this.#immediate(() => {
      ...
      const sourceHashesJson = currentSources ? JSON.stringify(this.#validateSnapshot(currentSources)) : null;
      const sourcesChanged = currentSources !== undefined && sourceHashesJson !== null
        && !this.#matchesSourceSnapshot(runId, currentSources, sourceHashesJson);
      const retryStage = sourcesChanged ? "analyzing" : run.failed_stage;
      const revision = run.current_revision + 1;
      this.#db.query("INSERT INTO revisions(run_id,revision,origin,source_revision,retry_stage,status,created_at) VALUES (?,?, 'retry',?,?,?,?)").run(run.id, revision, run.current_revision, retryStage, retryStage, now);
      if (sourcesChanged && currentSources && sourceHashesJson !== null) {
        this.#db.query("INSERT INTO run_source_snapshots(run_id,revision,manifest_sha256,baseline_sha256,source_hashes_json,created_at) VALUES (?,?,?,?,?,?)")
          .run(run.id, revision, currentSources.manifestSha256, currentSources.baselineSha256, sourceHashesJson, now);
      }
      this.#db.query("UPDATE runs SET current_revision=?,status=?,failed_stage=NULL,visual_ack_required=0,updated_at=? WHERE id=?").run(revision, retryStage, now, run.id);
      this.#event(run.id, revision, "run.retried", { status: retryStage, failedStage: run.failed_stage, sourceRevision: run.current_revision, sourcesChanged }, now);
      return publicRun(this.#run(run.id));
    });
  }

  retailor(runId: string, expectedPdfSha256: string, currentSources: RunSourceSnapshotInput): PublicRun {
    const sourceHashesJson = JSON.stringify(this.#validateSnapshot(currentSources));
    return this.#immediate(() => {
      const now = this.#now();
      const run = this.#assertCommandable(runId, now);
      this.#assertRunArtifactsRetained(run.id);
      if (run.status !== "review" && run.status !== "approved") {
        throw new RepositoryConflictError("only a completed run can be re-tailored");
      }
      if (run.application_status !== "pending") {
        throw new RepositoryConflictError("only a pending application can be re-tailored");
      }
      if (run.skip_review === 1 || run.auto_submit === 1) {
        throw new RepositoryConflictError("automatic application runs cannot be re-tailored");
      }
      const pdf = this.getArtifact(run.id, "compiled-pdf", run.current_revision);
      if (
        !pdf
        || pdf.sha256 !== expectedPdfSha256
        || (run.status === "approved" && run.approved_pdf_sha256 !== expectedPdfSha256)
      ) {
        throw new RepositoryConflictError("review PDF hash is stale");
      }
      ...
      if (
        latestApplication
        && (
          TERMINAL_APPLICATION_SESSION_STATES[latestApplication.bridge_state] !== true
          || latestApplication.slot_released !== 1
          || latestApplication.submission_phase !== "not_attempted"
        )
      ) {
        throw new RepositoryConflictError("application session is active or submission was attempted");
      }
      const sourcesChanged = !this.#matchesSourceSnapshot(run.id, currentSources, sourceHashesJson);
      const sourceRevision = run.current_revision;
      const targetRevision = sourceRevision + 1;
      this.#db.query("INSERT INTO revisions(run_id,revision,origin,source_revision,retry_stage,status,created_at) VALUES (?,?,'retry',?,'analyzing','analyzing',?)")
        .run(run.id, targetRevision, sourceRevision, now);
      this.#db.query("INSERT INTO run_source_snapshots(run_id,revision,manifest_sha256,baseline_sha256,source_hashes_json,created_at) VALUES (?,?,?,?,?,?)")
        .run(run.id, targetRevision, currentSources.manifestSha256, currentSources.baselineSha256, sourceHashesJson, now);
      this.#db.query("UPDATE runs SET current_revision=?,status='analyzing',failed_stage=NULL,approved_pdf_sha256=NULL,visual_ack_required=0,updated_at=? WHERE id=?")
        .run(targetRevision, now, run.id);
      this.#event(run.id, targetRevision, "run.retailored", {
        from: run.status,
        status: "analyzing",
        sourceRevision,
        sourcesChanged,
        approvalInvalidated: run.status === "approved",
      }, now);
      return publicRun(this.#run(run.id));
    });
  }
  ...
  approve(runId: string, expectedPdfSha256: string, visualAcknowledged = false): PublicRun {
    return this.#immediate(() => {
      const now = this.#now(); const run = this.#assertCommandable(runId, now);
      this.#assertRunArtifactsRetained(run.id);
