...
const migration24 = `
CREATE TABLE discovery_jobs_v24 (
  id TEXT PRIMARY KEY,
  catalog_source_id TEXT NOT NULL CHECK (length(catalog_source_id) BETWEEN 1 AND 200),
  catalog_source_item_id TEXT NOT NULL CHECK (length(catalog_source_item_id) BETWEEN 1 AND 500),
  title TEXT NOT NULL CHECK (length(title) BETWEEN 1 AND 500),
...
INSERT INTO discovery_jobs_v24(
...
INSERT INTO discovery_job_roles(job_id, role)
SELECT id, role
FROM discovery_jobs;

DROP TABLE discovery_jobs;
ALTER TABLE discovery_jobs_v24 RENAME TO discovery_jobs;
...
const migration29 = `
UPDATE run_application_sessions
SET public_snapshot_json = json_remove(public_snapshot_json, '$.browserUseDiagnostics')
WHERE json_type(public_snapshot_json, '$.browserUseDiagnostics') IS NOT NULL;
...
SET public_snapshot_json = json_set(
  ...
  (
    SELECT json_group_array(json_remove(value, '$.timedOut'))
    FROM (
      SELECT value
      FROM json_each(run_application_sessions.public_snapshot_json, '$.playwrightCliDiagnostics')
      ORDER BY CAST(key AS INTEGER)
    )
  )
)
WHERE json_type(public_snapshot_json, '$.playwrightCliDiagnostics') = 'array'
  AND EXISTS (
    SELECT 1
    FROM json_each(run_application_sessions.public_snapshot_json, '$.playwrightCliDiagnostics')
    WHERE json_type(value, '$.timedOut') IS NOT NULL
  );
`;
...
SET public_snapshot_json = json_set(
  ...
  (
    SELECT json_group_array(json(CASE
      WHEN json_extract(value, '$.status') = 'timed_out'
        AND json_extract(value, '$.errorCategory') = 'execution_timeout'
      THEN json_set(value, '$.status', 'failed', '$.errorCategory', 'process_exit', '$.stderrExcerpt', '[redacted]')
      ELSE value
    END))
    FROM (
      SELECT value
...
WHERE json_type(public_snapshot_json, '$.playwrightCliDiagnostics') = 'array'
  AND EXISTS (
    SELECT 1
    FROM json_each(run_application_sessions.public_snapshot_json, '$.playwrightCliDiagnostics')
    WHERE json_extract(value, '$.status') = 'timed_out'
      AND json_extract(value, '$.errorCategory') = 'execution_timeout'
  );
`;

const migration31 = `
DROP TABLE IF EXISTS discovery_run_links;
DROP TABLE IF EXISTS discovery_dedupe_keys;
DROP TABLE IF EXISTS discovery_observations;
DROP TABLE IF EXISTS discovery_job_roles;
DROP TABLE IF EXISTS discovery_jobs;
DROP TABLE IF EXISTS discovery_sources;
`;

function migrateApplicationOriginPauses(db: Database): void {
  const table = db.query<{ sql: string }, []>(
    "SELECT sql FROM sqlite_schema WHERE type = 'table' AND name = 'run_application_sessions'",
  ).get();
  if (!table) return;

  db.exec(`
    UPDATE run_application_sessions
    SET bridge_state = 'awaiting_human_navigation'
    WHERE bridge_state = 'awaiting_origin_approval'
  `);
  if (hasApplicationSessionSnapshotColumn(db)) {
    const rows = db.query<{ run_id: string; generation: number; public_snapshot_json: string }, []>(`
      SELECT run_id, generation, public_snapshot_json FROM run_application_sessions
      WHERE json_extract(public_snapshot_json, '$.bridgeState') = 'awaiting_origin_approval'
        OR json_extract(public_snapshot_json, '$.harnessState') = 'awaiting_origin_approval'
        OR json_extract(public_snapshot_json, '$.pendingAction.type') = 'origin_approval'
    `).all();
    const update = db.query(`
      UPDATE run_application_sessions SET public_snapshot_json = ?
      WHERE run_id = ? AND generation = ?
    `);
    for (const row of rows) {
      const snapshot = JSON.parse(row.public_snapshot_json) as Record<string, unknown>;
      if (snapshot.bridgeState === "awaiting_origin_approval") {
        snapshot.bridgeState = "awaiting_human_navigation";
      }
      if (snapshot.harnessState === "awaiting_origin_approval") {
        snapshot.harnessState = "awaiting_human_navigation";
      }
      const pending = snapshot.pendingAction as { type?: unknown } | null;
      if (snapshot.bridgeState === "awaiting_human_navigation") {
        snapshot.pendingAction = {
          type: "human_navigation",
          instruction: "Origin approval is no longer required. The browser session may need resuming; check the browser before continuing.",
        };
      } else if (pending?.type === "origin_approval") {
        // A terminal/lost projection must not acquire a new actionable pause.
        snapshot.pendingAction = null;
      }
      update.run(JSON.stringify(snapshot), row.run_id, row.generation);
    }
  }

  const upgradedSql = table.sql.replace(/'awaiting_origin_approval'\s*,\s*/g, "");
  if (upgradedSql === table.sql) return;
  const dependentObjects = db.query<{ sql: string }, []>(`
    SELECT sql FROM sqlite_schema
    WHERE tbl_name = 'run_application_sessions'
      AND type IN ('index', 'trigger') AND sql IS NOT NULL
    ORDER BY type, name
  `).all();
  db.exec(upgradedSql.replace(
    /^CREATE TABLE\s+(?:"run_application_sessions"|run_application_sessions)(?=\s*\()/i,
    "CREATE TABLE run_application_sessions_pending_migration",
  ));
  db.exec("INSERT INTO run_application_sessions_pending_migration SELECT * FROM run_application_sessions");
  db.exec("DROP TABLE run_application_sessions");
  db.exec("ALTER TABLE run_application_sessions_pending_migration RENAME TO run_application_sessions");
  for (const object of dependentObjects) db.exec(object.sql);
}
...
const migration33 = `
CREATE TABLE run_source_snapshots_v33 (
  run_id TEXT NOT NULL,
  revision INTEGER NOT NULL CHECK (revision > 0),
  manifest_sha256 TEXT NOT NULL CHECK (length(manifest_sha256) = 64),
  baseline_sha256 TEXT NOT NULL CHECK (length(baseline_sha256) = 64),
...
INSERT INTO run_source_snapshots_v33(run_id,revision,manifest_sha256,baseline_sha256,source_hashes_json,created_at)
SELECT run_id,1,manifest_sha256,baseline_sha256,source_hashes_json,created_at FROM run_source_snapshots;
DROP TABLE run_source_snapshots;
ALTER TABLE run_source_snapshots_v33 RENAME TO run_source_snapshots;
CREATE TRIGGER run_source_snapshots_no_update BEFORE UPDATE ON run_source_snapshots BEGIN SELECT RAISE(ABORT, 'source snapshots are immutable'); END;
CREATE TRIGGER run_source_snapshots_no_delete BEFORE DELETE ON run_source_snapshots BEGIN SELECT RAISE(ABORT, 'source snapshots are immutable'); END;
`;

const migration34 = `
CREATE TABLE application_model_selection (
  id INTEGER PRIMARY KEY CHECK (id = 1),
  model TEXT NOT NULL CHECK (model IN ('gpt-5.6-sol', 'gemini-3.8-flash'))
) STRICT;
`;
const lifecycleApplicationStatusCheck =
  /CHECK\s*\(\s*application_status\s+IN\s*\(\s*'pending'\s*,\s*'did_not_apply'\s*,\s*'applied'\s*,\s*'oa_received'\s*,\s*'oa_completed'\s*,\s*'rejected'\s*,\s*'interview'\s*,\s*'accepted'\s*,\s*'failed'\s*\)\s*\)/i;
const manualApplicationStatusCheck =
  /CHECK\s*\(\s*application_status\s+IN\s*\(\s*'pending'\s*,\s*'did_not_apply'\s*,\s*'manual_application'\s*,\s*'applied'\s*,\s*'oa_received'\s*,\s*'oa_completed'\s*,\s*'rejected'\s*,\s*'interview'\s*,\s*'accepted'\s*,\s*'failed'\s*\)\s*\)/i;

function migrateManualApplicationStatus(db: Database): void {
  const runsSql = db.query<{ sql: string | null }, []>(
    "SELECT sql FROM sqlite_schema WHERE type = 'table' AND name = 'runs'",
  ).get()?.sql;
  const hasApplicationStatus = db.query<{ name: string }, []>("PRAGMA table_info(runs)")
    .all()
    .some(({ name }) => name === "application_status");
  if (!hasApplicationStatus) return;
  if (!runsSql || !runsTableDeclaration.test(runsSql)) {
    throw new Error("runs application_status constraint does not match schema version 34");
  }
  if (manualApplicationStatusCheck.test(runsSql)) return;
  if (!lifecycleApplicationStatusCheck.test(runsSql)) {
    throw new Error("runs application_status constraint does not match schema version 34");
  }
  ...
  const upgradedRunsSql = runsSql
    .replace(runsTableDeclaration, "CREATE TABLE runs_pending_migration")
    .replace(
      lifecycleApplicationStatusCheck,
      "CHECK (application_status IN ('pending','did_not_apply','manual_application','applied','oa_received','oa_completed','rejected','interview','accepted','failed'))",
    );
  replaceRunsTable(db, upgradedRunsSql);
}

const migration36 = `
ALTER TABLE run_claim RENAME TO run_claim_v35;
...
INSERT INTO run_claim(id, run_id, claim_token, expires_at)
SELECT claim_slots.id, run_claim_v35.run_id, run_claim_v35.claim_token, run_claim_v35.expires_at
FROM claim_slots
LEFT JOIN run_claim_v35 ON run_claim_v35.id = claim_slots.id;

DROP TABLE run_claim_v35;
`;

function migrateExpandedRunClaimCapacity(db: Database): void {
  const existing = db.query<{ name: string }, []>(
    "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'run_claim'",
  ).get();
  if (!existing) {
...
export function migratePipelineDatabase(db: Database, now = Date.now()): void {
  const observedVersion = Number(db.query<{ user_version: number }, []>("PRAGMA user_version").get()?.user_version ?? 0);
  if (observedVersion > PIPELINE_SCHEMA_VERSION) throw new Error(`pipeline database version ${observedVersion} is newer than supported ${PIPELINE_SCHEMA_VERSION}`);
  if (observedVersion === PIPELINE_SCHEMA_VERSION) return;
  ...
  try {
    ...
    try {
      ...
      if (version < 29) {
        if (hasApplicationSessionSnapshotColumn(db)) db.exec(migration29);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(29, now);
      }
      if (version < 30) {
        if (hasApplicationSessionSnapshotColumn(db)) db.exec(migration30);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(30, now);
      }
      if (version < 31) {
        // Foreign keys remain disabled so historical dangling Discovery references
        // (including discovery_jobs_v24) do not prevent retiring the catalog.
        db.exec(migration31);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(31, now);
      }
      if (version < 32) {
        migrateApplicationOriginPauses(db);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(32, now);
      }
      if (version < 33) {
        db.exec(migration33);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(33, now);
      }
      if (version < 34) {
        db.exec(migration34);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(34, now);
      }
      if (version < 35) {
        migrateManualApplicationStatus(db);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(35, now);
      }
      if (version < 36) {
        migrateExpandedRunClaimCapacity(db);
        db.query("INSERT INTO schema_migrations(version, applied_at) VALUES (?, ?)").run(36, now);
      }
      // Validate the final schema, after retiring Discovery and its legacy dangling references.
      const foreignKeyFailures = db.query<{ table: string }, []>("PRAGMA foreign_key_check").all();
      if (foreignKeyFailures.length > 0) throw new Error("foreign key integrity check failed after pipeline migration");
