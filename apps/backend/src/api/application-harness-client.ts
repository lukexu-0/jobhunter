...
export interface ApplicationHarnessCreateInput {
  ...
  readonly transcriptPdf?: Uint8Array;
...
const RawSnapshotSchema = z.object({
  ...
  model_provider: z.enum(["openai-codex", "google-antigravity"]),
  model: z.enum(["gpt-5.6-sol", "gemini-3.8-flash"]),
  reasoning: z.enum(["medium", "high"]),
  ...
  if ((snapshot.model_provider === "openai-codex") !== (snapshot.model === "gpt-5.6-sol")) {
    context.addIssue({ code: "custom", path: ["model"], message: "model does not match provider" });
  }
  const expectedReasoning = snapshot.model_provider === "openai-codex" ? "medium" : "high";
  if (snapshot.reasoning !== expectedReasoning) {
    context.addIssue({ code: "custom", path: ["reasoning"], message: "reasoning does not match provider" });
  }
...
export interface ModelAuthHarnessClient {
  setModelCredential(provider: ModelAuthProvider, credential: OAuthCredential, signal: AbortSignal): Promise<void>;
  deleteModelCredential(provider: ModelAuthProvider, signal: AbortSignal): Promise<void>;
  setApplicationModel(model: ApplicationModelId, signal: AbortSignal): Promise<void>;
}
...
const GmailAuthSessionResponseSchema = z.object({
  ...
  authorization_url: z.string().max(8_192).refine(isGmailAuthorizationUrl).optional(),
...
function pythonSplitLineCount(value: string): number {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    const isSeparator = codeUnit === 10
      || codeUnit === 11
      || codeUnit === 12
      || codeUnit === 13
      || (codeUnit >= 28 && codeUnit <= 30)
      || codeUnit === 133
      || codeUnit === 8_232
      || codeUnit === 8_233;
    if (!isSeparator) {
    ...
    if (codeUnit === 13 && value.charCodeAt(index + 1) === 10) index += 1;
  }
  return lineCount + (endsWithSeparator ? 0 : 1);
}

const SourceCaptureCompleteResponseSchema = z.object({
  capture_id: UUIDSchema,
  final_url: z.string().max(SOURCE_CAPTURE_FINAL_URL_MAX_CHARACTERS),
  source: z.string().min(1),
}).strict().superRefine((value, context) => {
  if (!isCanonicalSourceCaptureHttpsUrl(
    value.final_url,
    SOURCE_CAPTURE_FINAL_URL_MAX_CHARACTERS,
  )) {
    context.addIssue({
      code: "custom",
      path: ["final_url"],
  ...
  if (pythonSplitLineCount(value.source) > LUNA_MAX_SOURCE_LINES) {
    context.addIssue({
      code: "custom",
      path: ["source"],
...
function snapshotOptionalTranscriptPdf(value: unknown): Uint8Array<ArrayBuffer> | null | undefined {
  ...
  try {
    ...
    if (
      ...
      || byteLength < 5
      || byteLength > APPLICATION_SESSION_UPLOAD_LIMITS.transcriptPdfBytes
    ) return null;
    const snapshot = new Uint8Array(byteLength);
    Uint8Array.prototype.set.call(snapshot, value);
    if (String.fromCharCode(...snapshot.subarray(0, 5)) !== "%PDF-") return null;
    return snapshot;
  } catch {
    return null;
  }
...
async function mapGmailErrorResponse(
  response: Response,
  signal: AbortSignal,
): Promise<never> {
  ...
  if (
    response.status === 503
    && envelope.success
    && envelope.data.code === "gmail_oauth_unavailable"
  ) {
...
async function mapErrorResponse(
  ...
  if (response.status === 404 && envelope.data.code === "question_not_found") {
...
export class HttpApplicationHarnessClient implements
  ...
  async createGmailAuthSession(signal: AbortSignal): Promise<GmailAuthSession> {
    ...
    if (!response.ok || response.status !== 201) {
      return mapGmailErrorResponse(response, signal);
    }
  ...
  async setModelCredential(
    ...
    const parsedProvider = ModelAuthProviderSchema.safeParse(provider);
    if (!parsedProvider.success || credential.type !== "oauth") {
      throw new ApplicationHarnessError("invalid_request");
    }
    const response = await this.#request(
      `/v1/model-credentials/${parsedProvider.data}`,
      {
        method: "PUT",
  ...
  async deleteModelCredential(provider: ModelAuthProvider, signal: AbortSignal): Promise<void> {
    const parsedProvider = ModelAuthProviderSchema.safeParse(provider);
    if (!parsedProvider.success) throw new ApplicationHarnessError("invalid_request");
    const response = await this.#request(
      `/v1/model-credentials/${parsedProvider.data}`,
      { method: "DELETE" },
      signal,
  ...
  async setApplicationModel(model: ApplicationModelId, signal: AbortSignal): Promise<void> {
    const parsedModel = ApplicationModelIdSchema.safeParse(model);
    if (!parsedModel.success) throw new ApplicationHarnessError("invalid_request");
    const response = await this.#request(
      "/v1/application-model",
      {
        method: "PUT",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ model: parsedModel.data }),
      },
      signal,
    );
    if (!response.ok || response.status !== 204) {
  ...
  async create(input: ApplicationHarnessCreateInput, signal: AbortSignal): Promise<void> {
    ...
    const transcriptPdf = snapshotOptionalTranscriptPdf(input.transcriptPdf);
    if (
      !sessionId.success
      || !opportunityKind.success
      || !isHarnessJobUrl(input.jobUrl)
      || typeof input.autoSubmit !== "boolean"
      || typeof input.autoEnd !== "boolean"
      || profileBytes < 1
    ...
    form.set("auto_end", input.autoEnd ? "true" : "false");
    form.set(
    ...
    form.set(
      "resume",
      new File([Uint8Array.from(input.resumePdf).buffer], "Applicant_Resume.pdf", { type: "application/pdf" }),
    );
    if (transcriptPdf !== undefined) {
      form.set(
        "transcript",
        new File(
          [transcriptPdf],
          "Applicant_Transcript.pdf",
  ...
  async open(sessionId: string, signal: AbortSignal): Promise<void> {
    ...
    const response = await this.#request(
      `/v1/sessions/${parsedSessionId.data}/browser/open`,
      { method: "POST" },
      signal,
    );
    if (!response.ok) await mapErrorResponse(response, parsedSessionId.data, signal);
  ...
  async createSourceCapture(
    ...
    if (
      !captureId.success
      || !isCanonicalSourceCaptureHttpsUrl(
        jobUrl,
        SOURCE_CAPTURE_CREATE_URL_MAX_CHARACTERS,
      )
    ) {
      throw new SourceCaptureHarnessError("invalid_request");
    }
