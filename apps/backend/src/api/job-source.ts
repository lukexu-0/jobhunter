...
import { RenderJobSourceError, renderJobSourceWithChrome, type RenderedJobSource } from "./rendered-job-source";
...
export type RenderJobSourceHtml = (url: string, signal: AbortSignal) => Promise<RenderedJobSource | undefined>;
...
>;
type LoadedModelFallback = Extract<LoadedJobSource, { readonly kind: "model-fallback" }>;
...
const ERROR_DETAILS = {
  ...
  JOB_SOURCE_RENDER_UNAVAILABLE: [
    422,
    "The local browser could not securely load this opportunity page. Run doctor to check Chrome and browser isolation, or paste the description.",
  ],
  JOB_HUMAN_VERIFICATION_REQUIRED: [
    409,
...
const MAX_GREENHOUSE_SCRIPTS = 24;
const MAX_GREENHOUSE_SCRIPT_BYTES = 256 * 1024;
...
const REMOVED_CONTENT_SELECTOR = [
  ...
  "nav", "body > header", "body > footer", "input", "button", "[hidden]", '[aria-hidden="true"]',
].join(", ");
...
async function sanitizeHtml(html: string, preserveFormContent = false): Promise<string> {
  const rewriter = new HTMLRewriter()
    .on(REMOVED_CONTENT_SELECTOR, {
      element(element) { element.remove(); },
    })
    .on("form", {
      element(element) {
        if (preserveFormContent) element.removeAndKeepContent();
        else element.remove();
      },
    });
...
async function collectInlineScripts(html: string): Promise<string[]> {
...
function parsePhenomDdo(script: string): Record<string, unknown> | undefined {
  const assignment = /(?:^|[;\s])phApp\.ddo\s*=\s*/g;
  ...
  for (let index = start; index < script.length; index += 1) {
    ...
    if (character === '"') {
      ...
      if (assignment.exec(script)) return undefined;
      try {
        const value: unknown = JSON.parse(script.slice(start, index + 1));
...
async function loadPhenomLeadPageFallback(
  html: string,
  opportunityKindHint?: OpportunityKind,
): Promise<LoadedJobSource | undefined> {
  if (!html.includes("phApp.ddo")) return undefined;
  const candidates = (await collectInlineScripts(html))
    .map(parsePhenomDdo)
    .filter((value): value is Record<string, unknown> => value !== undefined);
  if (candidates.length !== 1) return undefined;
  const ddo = candidates[0]!;
  const registration = asRecord(ddo.crmEventRegisterForm) ?? asRecord(ddo.crmEventRegisterFormV2);
  const event = asRecord(registration?.eventDetails);
  const languages = Array.isArray(event?.eLanguages)
    ? event.eLanguages.map(asRecord).filter((value): value is Record<string, unknown> => value !== undefined)
    : [];
  const defaults = languages.filter((language) => language.default === true || language.isDefault === true);
  const language = defaults.length === 1
    ? defaults[0]
    : defaults.length === 0 && languages.length === 1
      ? languages[0]
      : undefined;
  if (!language || typeof language.title !== "string") return undefined;
  const description = typeof language.description === "string" && language.description.trim()
    ? language.description
    : typeof language.teaser === "string" && language.teaser.trim()
      ? language.teaser
      : undefined;
  if (description === undefined) return undefined;
  return providerDescription(language.title, [description], opportunityKindHint);
}

const SUPERHUMAN_ASHBY_BOARD = "Superhuman Platform Inc";
const ASHBY_JOB_ID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

async function loadSuperhumanAshbyFallback(
  sourceUrl: string,
  html: string,
  signal: AbortSignal,
  ...
  const jobIds = url.searchParams.getAll("ashby_jid");
  if (
    url.origin !== "https://superhuman.com"
    || url.pathname !== "/company/careers/jobs"
    || jobIds.length !== 1
    || !ASHBY_JOB_ID_PATTERN.test(jobIds[0]!)
  ) return undefined;

  let hasAshbyEmbed = false;
  await new HTMLRewriter().on("#ashby_embed", {
    element() { hasAshbyEmbed = true; },
  }).transform(new Response(html)).text();
  if (!hasAshbyEmbed) return undefined;

  const canonicalUrl = new URL(
    `https://jobs.ashbyhq.com/${encodeURIComponent(SUPERHUMAN_ASHBY_BOARD)}/${jobIds[0]}`,
  );
  return loadJobSourceWithSignal(
    canonicalUrl.href,
    signal,
    fetchImpl,
    resolveHost,
...
function buildFallbackCandidate(candidate: string): LoadedModelFallback | "too-large" | undefined {
...
async function fallbackFromSanitizedHtml(
  sanitized: string,
): Promise<LoadedModelFallback | "too-large" | undefined> {
  ...
  let longestCandidate: LoadedModelFallback | undefined;
  let longestLength = -1;
  ...
  return longestCandidate ?? (sawOversized ? "too-large" : undefined);
}

async function htmlFallback(html: string): Promise<LoadedModelFallback> {
  const primary = await fallbackFromSanitizedHtml(await sanitizeHtml(html));
  if (primary && primary !== "too-large") return primary;
  if (primary === "too-large") throw new JobSourceError("JOB_SOURCE_TOO_LARGE");

  const withForms = await fallbackFromSanitizedHtml(await sanitizeHtml(html, true));
  if (withForms && withForms !== "too-large") return withForms;
  throw new JobSourceError(withForms === "too-large" ? "JOB_SOURCE_TOO_LARGE" : "JOB_DESCRIPTION_UNAVAILABLE");
}
...
function greenhouseSource(url: URL): GreenhouseSource | undefined {
  if (url.protocol !== "https:" || url.port || url.username || url.password
    || (url.hostname !== "boards.greenhouse.io" && url.hostname !== "job-boards.greenhouse.io")) return undefined;
  const standard = /^\/([A-Za-z0-9_-]{1,100})\/jobs\/([0-9]{1,20})\/?$/.exec(url.pathname);
  let board = standard?.[1];
  let jobId = standard?.[2];
  if (!standard && url.pathname === "/embed/job_app") {
    if (url.searchParams.getAll("for").length !== 1 || url.searchParams.getAll("token").length !== 1) return undefined;
    board = url.searchParams.get("for")!;
    jobId = url.searchParams.get("token")!;
    const jobHints = url.searchParams.getAll("gh_jid");
    if (jobHints.some((hint) => hint !== jobId)) return undefined;
  }
  if (!board || !/^[A-Za-z0-9_-]{1,100}$/.test(board) || !jobId || !/^[0-9]{1,20}$/.test(jobId)) return undefined;
  return { endpoint: new URL(`https://boards-api.greenhouse.io/v1/boards/${board}/jobs/${jobId}`), jobId };
}

function greenhouseBoardLoaderSource(url: URL, jobId: string): GreenhouseSource | undefined {
  if (url.protocol !== "https:" || url.port || url.username || url.password
    || url.hostname !== "boards.greenhouse.io" || url.pathname !== "/embed/job_board/js"
    || !/^[0-9]{1,20}$/.test(jobId)) return undefined;
  const boards = url.searchParams.getAll("for");
  if (boards.length !== 1 || !/^[A-Za-z0-9_-]{1,100}$/.test(boards[0]!)) return undefined;
  return {
    endpoint: new URL(`https://boards-api.greenhouse.io/v1/boards/${boards[0]!}/jobs/${jobId}`),
    jobId,
  };
}

async function providerDescription(title: unknown, fragments: readonly unknown[], opportunityKindHint?: OpportunityKind): Promise<LoadedJobSource> {
  const normalized: string[] = [];
  for (const fragment of fragments) {
    if (typeof fragment !== "string") continue;
    const text = await normalizeHtmlFragment(fragment);
    if (text) normalized.push(text);
  }
  const description = normalized.join("\n\n");
  if (description.length < JOB_DESCRIPTION_MIN_CHARS) throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  const normalizedTitle = typeof title === "string" ? await normalizeHtmlFragment(title) : "";
  const text = normalizedTitle ? `${normalizedTitle}\n\n${description}` : description;
  if (opportunityKindHint !== undefined) {
    const parsed = JobDescriptionSchema.safeParse(text);
    if (!parsed.success) throw new JobSourceError(text.length > JOB_DESCRIPTION_MAX_CHARS ? "JOB_SOURCE_TOO_LARGE" : "JOB_DESCRIPTION_UNAVAILABLE");
    return { kind: "description", opportunityKind: opportunityKindHint, jobDescription: parsed.data };
  }
  const candidate = buildFallbackCandidate(text);
...
async function isDynamicGreenhousePlaceholderPage(sourceUrl: string, html: string): Promise<boolean> {
  const url = new URL(sourceUrl);
  const jobIds = url.searchParams.getAll("gh_jid");
  if (url.protocol !== "https:" || jobIds.length !== 1 || !/^[0-9]{1,20}$/.test(jobIds[0]!)) return false;

  let placeholders = 0;
  await new HTMLRewriter()
    .on("[job-title]", { element() { placeholders |= 1; } })
    .on("[job-content]", { element() { placeholders |= 2; } })
    .on("[job-form]", { element() { placeholders |= 4; } })
    .transform(new Response(html)).text();
  return placeholders === 7;
}

async function loadGreenhouseScriptFallback(
  sourceUrl: string,
  html: string,
  signal: AbortSignal,
  fetchImpl: JobSourceFetch,
  resolveHost: ResolveHost,
  dynamicPlaceholderPage: boolean,
  opportunityKindHint?: OpportunityKind,
): Promise<LoadedJobSource | undefined> {
  const pageUrl = new URL(sourceUrl);
  const jobIds = pageUrl.searchParams.getAll("gh_jid");
  if (pageUrl.protocol !== "https:" || jobIds.length !== 1 || !/^[0-9]{1,20}$/.test(jobIds[0]!)) return undefined;
  const jobId = jobIds[0]!;
  const sources = new Map<string, GreenhouseSource>();
  const scripts = new Set<string>();
  const addScript = (url: URL) => {
    url.hash = "";
    scripts.add(url.href);
    if (scripts.size > MAX_GREENHOUSE_SCRIPTS) throw new JobSourceError("JOB_SOURCE_TOO_LARGE");
  };
  const inspect = (text: string) => {
    // Read only quoted literal URLs, never evaluate source or follow module imports.
    for (const match of text.matchAll(/(["'`])(https:\/\/(?:boards|job-boards)\.greenhouse\.io\/embed\/job_app\?[^"'`\s\\]*)\1/g)) {
      const url = new URL(match[2]!);
      const tokens = url.searchParams.getAll("token");
      if (tokens.length !== 1 || url.searchParams.getAll("for").length !== 1) {
        throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
      }
      if (/^\$\{[A-Za-z_$][A-Za-z0-9_$]*\}$/.test(tokens[0]!)) url.searchParams.set("token", jobId);
      const source = greenhouseSource(url);
      if (!source) continue;
      if (source.jobId !== jobId) throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
      sources.set(source.endpoint.href, source);
    }
  };
  const discoverDynamicScripts = (text: string) => {
    // Follow only quoted literal HTTPS scripts declared by the page; never evaluate source.
    for (const match of text.matchAll(/(["'])(https:\/\/[^"'\s\\]+)\1(?!\s*\+)/g)) {
      let url: URL;
      try { url = new URL(match[2]!); } catch { continue; }
      if (url.username || url.password || url.port || !url.pathname.endsWith(".js")) continue;
      addScript(url);
    }
  };
  ...
  await new HTMLRewriter().on("script", {
    element(element) {
      ...
      if (src !== null) {
        if (dynamicPlaceholderPage) return;
        let url: URL;
        try { url = new URL(decodeHtmlEntities(src), pageUrl); } catch { return; }
        if (url.protocol !== "https:" || url.origin !== pageUrl.origin || url.username || url.password) return;
        addScript(url);
        return;
      }
      inline = [];
      element.onEndTag(() => {
        if (inline) {
          ...
          inspect(text);
          if (dynamicPlaceholderPage) discoverDynamicScripts(text);
        }
        inline = undefined;
      });
    },
    text(text) { inline?.push(text.text); },
  }).transform(new Response(html)).text();
  // Sequential bounded reads cap downloaded script bodies at 24 × 256 KiB (6 MiB).
  for (const scriptUrl of scripts) {
    const { response } = await fetchPinnedPublicHttp(scriptUrl, { signal, fetchImpl, resolveHost });
    try {
      if (response.status < 200 || response.status > 299) throw new JobSourceError("JOB_SOURCE_UNAVAILABLE");
      const encoding = response.headers.get("content-encoding");
      if (encoding && encoding.trim().toLowerCase() !== "identity") throw new JobSourceError("JOB_SOURCE_UNAVAILABLE");
      const length = response.headers.get("content-length");
      if (length && /^\d+$/.test(length.trim()) && Number(length) > MAX_GREENHOUSE_SCRIPT_BYTES) throw new JobSourceError("JOB_SOURCE_TOO_LARGE");
    } catch (error) {
      cancelPublicHttpBody(response);
      throw error;
    }
    const bytes = await readBoundedPublicHttpBody(response, signal, MAX_GREENHOUSE_SCRIPT_BYTES);
    inspect(new TextDecoder("utf-8", { fatal: true }).decode(bytes));
  }
  if (sources.size === 0) return undefined;
  if (sources.size !== 1) throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  return loadGreenhouseJob(sources.values().next().value!, signal, fetchImpl, resolveHost, opportunityKindHint);
}

async function loadGreenhouseFallback(
  sourceUrl: string,
  html: string,
  signal: AbortSignal,
  fetchImpl: JobSourceFetch,
  ...
  const pageUrl = new URL(sourceUrl);
  const sources = new Map<string, GreenhouseSource>();
  const direct = greenhouseSource(pageUrl);
  if (direct) sources.set(direct.endpoint.href, direct);
  const jobIds = pageUrl.searchParams.getAll("gh_jid");
  const boardLoaderJobId = jobIds.length === 1 && /^[0-9]{1,20}$/.test(jobIds[0]!) ? jobIds[0] : undefined;
  await new HTMLRewriter()
    .on("iframe[src]", {
      element(element) {
        try {
          const source = greenhouseSource(new URL(decodeHtmlEntities(element.getAttribute("src")!), pageUrl));
          if (source) sources.set(source.endpoint.href, source);
        } catch { /* Invalid iframe URLs are not provider sources. */ }
      },
    })
    .on("script[src]", {
      element(element) {
        if (!boardLoaderJobId) return;
        try {
          const source = greenhouseBoardLoaderSource(
            new URL(decodeHtmlEntities(element.getAttribute("src")!), pageUrl),
            boardLoaderJobId,
          );
          if (source) sources.set(source.endpoint.href, source);
        } catch { /* Invalid script URLs are not provider sources. */ }
      },
    })
    .transform(new Response(html)).text();
  if (sources.size === 0) return undefined;
  ...
  if (pageUrl.searchParams.getAll("gh_jid").some((id) => id !== source.jobId)) {
    throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  }
  return loadGreenhouseJob(source, signal, fetchImpl, resolveHost, opportunityKindHint);
}

async function loadGreenhouseJob(
  ...
  const document = await loadPublicJson(source.endpoint, signal, fetchImpl, resolveHost, { "application/json": true });
  if (!document || typeof document !== "object" || Array.isArray(document)) throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  const record = document as Record<string, unknown>;
  if ((typeof record.id !== "string" && (typeof record.id !== "number" || !Number.isSafeInteger(record.id)))
    || String(record.id) !== source.jobId || typeof record.content !== "string" || !record.content.trim()) {
    throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  }
  return providerDescription(record.title, [decodeHtmlEntities(record.content)], opportunityKindHint);
}

interface IcimsSource {
  readonly sourceUrl: URL;
  readonly jobId: string;
  readonly inIframe: boolean;
}

function icimsSource(sourceUrl: string): IcimsSource | undefined {
  const url = new URL(sourceUrl);
  const hostname = url.hostname.toLowerCase();
  if (url.protocol !== "https:" || url.port || url.username || url.password || !hostname.endsWith(".icims.com")) {
    return undefined;
  }
  const match = /^\/jobs\/([1-9][0-9]{0,19})(?:\/[^/]{1,512})?\/job\/?$/.exec(url.pathname);
  if (!match) return undefined;
  const iframeValues = url.searchParams.getAll("in_iframe");
  return {
    sourceUrl: url,
    jobId: match[1]!,
    inIframe: iframeValues.length === 1 && iframeValues[0] === "1",
  };
}

async function loadIcimsFallback(
  loadedSource: LoadedPublicWebSource,
  signal: AbortSignal,
  fetchImpl: JobSourceFetch,
  resolveHost: ResolveHost,
  opportunityKindHint?: OpportunityKind,
): Promise<LoadedJobSource | undefined> {
  const source = icimsSource(loadedSource.url);
  if (!source) return undefined;
  if (await isTalHumanVerificationPage(loadedSource.body)) {
    throw new JobSourceError("JOB_HUMAN_VERIFICATION_REQUIRED");
  }
  let loaded = loadedSource;
  if (!source.inIframe) {
    const iframeUrl = new URL(source.sourceUrl);
    iframeUrl.hash = "";
    iframeUrl.searchParams.set("in_iframe", "1");
    loaded = await loadPublicWebSourceWithSignal(iframeUrl.href, signal, fetchImpl, resolveHost);
  }
  const finalSource = icimsSource(loaded.url);
  if (
    loaded.mediaType !== "html"
    || !finalSource?.inIframe
    || finalSource.sourceUrl.origin !== source.sourceUrl.origin
    || finalSource.jobId !== source.jobId
  ) {
    throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  }
  if (!source.inIframe && await isTalHumanVerificationPage(loaded.body)) {
    throw new JobSourceError("JOB_HUMAN_VERIFICATION_REQUIRED");
  }
  if (!source.inIframe) {
  ...
  const fallback = await htmlFallback(loaded.body);
  if (opportunityKindHint === undefined) return fallback;
  const jobDescription = fallback.lines.join("\n");
  const parsed = JobDescriptionSchema.safeParse(jobDescription);
  if (!parsed.success) {
    throw new JobSourceError(jobDescription.length > JOB_DESCRIPTION_MAX_CHARS
      ? "JOB_SOURCE_TOO_LARGE"
      : "JOB_DESCRIPTION_UNAVAILABLE");
  }
  return { kind: "description", opportunityKind: opportunityKindHint, jobDescription: parsed.data };
...
async function loadWorkableFallback(
  sourceUrl: string,
  signal: AbortSignal,
  fetchImpl: JobSourceFetch,
  resolveHost: ResolveHost,
  ...
  const url = new URL(sourceUrl);
  if (url.protocol !== "https:" || url.port || url.username || url.password || url.hostname !== "apply.workable.com") return undefined;
  const match = /^\/([A-Za-z0-9_-]{1,100})\/j\/([A-Za-z0-9_-]{1,32})\/?$/.exec(url.pathname);
  if (!match) return undefined;
  const endpoint = new URL(`https://apply.workable.com/api/v2/accounts/${match[1]}/jobs/${match[2]}`);
  const document = await loadPublicJson(endpoint, signal, fetchImpl, resolveHost, { "application/json": true });
  if (!document || typeof document !== "object" || Array.isArray(document)) throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  const record = document as Record<string, unknown>;
  if (record.shortcode !== match[2] || record.state !== "published" || typeof record.title !== "string"
    || ![record.description, record.requirements, record.benefits].some((part) => typeof part === "string" && part.trim())) {
    throw new JobSourceError("JOB_DESCRIPTION_UNAVAILABLE");
  }
  return providerDescription(record.title, [record.description, record.requirements, record.benefits], opportunityKindHint);
}
...
async function loadPublicJson(
  ...
  fetchImpl: JobSourceFetch,
  resolveHost: ResolveHost,
  acceptedTypes: Readonly<Record<string, true>>,
): Promise<unknown> {
  ...
  try {
    ...
    const type = rawType?.split(";", 1)[0]!.trim().toLowerCase();
    if (!type || !Object.hasOwn(acceptedTypes, type)) {
...
async function loadOracleCandidateExperienceFallback(
  ...
  const document = await loadPublicJson(apiUrl, signal, fetchImpl, resolveHost, ACCEPTED_ORACLE_JSON_TYPES);
...
function fetchInit(
  ...
  if (!headers.has("user-agent")) headers.set("user-agent", "lowser/0.1");
...
async function loadJobSourceWithSignal(
  ...
  const browserHandoffEligible = submittedInputUrl.protocol === "https:"
  ...
  if (deterministic === "too-large") throw new JobSourceError("JOB_SOURCE_TOO_LARGE");
  if (deterministic !== undefined) return deterministic;
  const superhumanAshby = await loadSuperhumanAshbyFallback(
    loaded.url,
    loaded.body,
    signal,
    fetchImpl,
  ...
  if (superhumanAshby !== undefined) return superhumanAshby;
  const phenom = await loadPhenomLeadPageFallback(loaded.body, opportunityKindHint);
  if (phenom !== undefined) return phenom;
  const icims = await loadIcimsFallback(loaded, signal, fetchImpl, resolveHost, opportunityKindHint);
  if (icims !== undefined) return icims;
  const workable = await loadWorkableFallback(loaded.url, signal, fetchImpl, resolveHost, opportunityKindHint);
  if (workable !== undefined) return workable;
  const greenhouse = await loadGreenhouseFallback(loaded.url, loaded.body, signal, fetchImpl, resolveHost, opportunityKindHint);
  if (greenhouse !== undefined) return greenhouse;
  ...
  if (
    browserHandoffEligible
    && await isTalHumanVerificationPage(loaded.body)
  ) {
    throw new JobSourceError("JOB_HUMAN_VERIFICATION_REQUIRED");
  }
  const dynamicGreenhousePlaceholderPage = await isDynamicGreenhousePlaceholderPage(loaded.url, loaded.body);
  if (!dynamicGreenhousePlaceholderPage) {
    try {
  ...
  const scriptGreenhouse = await loadGreenhouseScriptFallback(
    loaded.url,
    loaded.body,
    signal,
    fetchImpl,
    resolveHost,
    dynamicGreenhousePlaceholderPage,
    opportunityKindHint,
  );
  if (scriptGreenhouse !== undefined) return scriptGreenhouse;

  let rendered: RenderedJobSource | undefined;
  try {
    rendered = await hardRace(renderHtml(loaded.url, signal), signal);
  } catch (error) {
    if (signal.aborted && !(signal.reason instanceof DeadlineExpired)) throw cancellationReason(signal);
    // Only the renderer boundary is translated; provider and parsing errors remain visible.
    if (error instanceof RenderJobSourceError || !(error instanceof JobSourceError)) {
      throw new JobSourceError("JOB_SOURCE_RENDER_UNAVAILABLE");
    }
    throw error;
  }
  if (rendered === undefined) throw new JobSourceError("JOB_SOURCE_RENDER_UNAVAILABLE");
  const renderedHtml = rendered.html;
  if (Buffer.byteLength(renderedHtml, "utf8") > MAX_BODY_BYTES) throw new JobSourceError("JOB_SOURCE_TOO_LARGE");
  const renderedDeterministic = await deterministicHtmlDescription(renderedHtml, opportunityKindHint);
  if (renderedDeterministic === "too-large") throw new JobSourceError("JOB_SOURCE_TOO_LARGE");
  if (renderedDeterministic !== undefined) return renderedDeterministic;
  const renderedGreenhouse = await loadGreenhouseFallback(loaded.url, renderedHtml, signal, fetchImpl, resolveHost, opportunityKindHint);
  if (renderedGreenhouse !== undefined) return renderedGreenhouse;
  if (browserHandoffEligible && await isTalHumanVerificationPage(renderedHtml)) {
    throw new JobSourceError("JOB_HUMAN_VERIFICATION_REQUIRED");
  }
  return hardRace(htmlFallback(renderedHtml), signal);
}
...
export async function loadJobSourceFromUrl(
  ...
  const renderHtml: RenderJobSourceHtml = options.renderHtml ?? ((url, renderSignal) => renderJobSourceWithChrome(
    url, renderSignal,
    { resolveAddresses: (hostname, resolveSignal) => resolveValidatedHostnameAddresses(hostname, resolveHost, resolveSignal) },
  ));
  try {
