...
const PROFILE_RELATIVE_PATH = ".jobhunt-data/user-info/current-context/personal/applicant-profile.md";
...
export class ApplicationSessionServiceError extends Error {
  ...
  constructor(readonly code: ApplicationSessionServiceErrorCode, message?: string) {
...
export type ApplicantTranscriptReader = () =>
...
export interface ApplicationSessionServiceDependencies {
  ...
  readonly transcriptReader?: ApplicantTranscriptReader;
...
function validateTranscriptPdf(value: unknown): Uint8Array | undefined {
  if (value === undefined) return undefined;
  if (
    !(value instanceof Uint8Array)
    || value.byteLength < 5
    || value.byteLength > APPLICATION_SESSION_UPLOAD_LIMITS.transcriptPdfBytes
    || String.fromCharCode(...value.subarray(0, 5)) !== "%PDF-"
  ) {
    throw new Error("applicant transcript is not a bounded PDF");
  }
  return value;
}
...
export async function readApplicantTranscriptPdf(
  path = process.env.JOBHUNT_TRANSCRIPT_PDF,
): Promise<Uint8Array | undefined> {
  if (path === undefined || path === "") return undefined;
  if (!isAbsolute(path)) throw new Error("applicant transcript path must be absolute");
  ...
  if (pathStat.isSymbolicLink()) {
    throw new Error("applicant transcript path must not be a symlink");
  }
  if (
    !pathStat.isFile()
    || pathStat.size < 5
    || pathStat.size > APPLICATION_SESSION_UPLOAD_LIMITS.transcriptPdfBytes
  ) {
    throw new Error("applicant transcript must be a bounded regular file");
  }
  ...
  try {
    ...
    if (
      !before.isFile()
      || before.size < 5n
      || before.size > BigInt(APPLICATION_SESSION_UPLOAD_LIMITS.transcriptPdfBytes)
    ) {
      throw new Error("applicant transcript must be a bounded regular file");
    }
    const expectedSize = Number(before.size);
    const buffer = new Uint8Array(expectedSize + 1);
    let offset = 0;
    while (offset < buffer.byteLength) {
      const item = await handle.read(buffer, offset, buffer.byteLength - offset, null);
      if (item.bytesRead === 0) break;
    ...
    if (
      ...
      throw new Error("applicant transcript changed while reading");
    }
    return validateTranscriptPdf(buffer.subarray(0, offset));
  } finally {
    await handle.close();
  }
}
...
function durableBridgeState(
  ...
  switch (session.submissionPhase) {
    case "attempting":
      ...
      return (harnessState ?? session.bridgeState) === "awaiting_human_navigation"
...
export class ApplicationSessionService {
  ...
  constructor(private readonly dependencies: ApplicationSessionServiceDependencies) {
    ...
    this.#transcriptReader = dependencies.transcriptReader ?? (() => readApplicantTranscriptPdf());
  }
  ...
  async start(
    ...
    if (latest) {
      const run = isTerminal(latest)
        ...
        : null;
      const reapplyingSubmitted = options.reapply === true
        && latest.submissionPhase === "submitted"
        && run?.applicationStatus === "applied";
      if (submissionCannotRetry(latest) && !reapplyingSubmitted) {
        throw new ApplicationSessionServiceError("APPLICATION_SUBMISSION_FINAL");
      }
      if (
        run
        ...
        const prepared = await this.#prepareStart(runId, expectedApprovedPdfSha256, signal, options);
        ...
        try {
          reserved = this.dependencies.repository.reserveApplicationSession(
            ...
            { allowSubmittedApplication: reapplyingSubmitted },
          );
        } catch (error) {
          ...
          if (
            ...
            && concurrent.resumeRevision === prepared.resumeRevision
      ...
      return await this.#resume(
        runId,
        latest,
        () => this.#prepareStart(runId, expectedApprovedPdfSha256, signal, options),
        signal,
      );
    }
  ...
  async retry(
    ...
    if (!isTerminal(previous) || !previous.slotReleased) {
      if (submissionCannotRetry(previous)) {
        throw new ApplicationSessionServiceError("APPLICATION_SUBMISSION_FINAL");
      }
      throw new RunServiceError("RUN_CONFLICT", "application session is not terminal", 409);
    }
  ...
  async openBrowser(runId: string, signal: AbortSignal): Promise<void> {
    signal.throwIfAborted();
    let session: PublicApplicationSession | null;
    try {
      session = this.dependencies.repository.getLatestApplicationSession(runId);
    ...
    if (!harness) throw applicationHarnessUnavailable();
    try {
      await harness.open(session.sessionId, signal);
    } catch (error) {
      if (signal.aborted) signal.throwIfAborted();
      if (
        error instanceof ApplicationHarnessError
    ...
    if (
      ...
      || (
        ...
        && !(
          ...
          && session.bridgeState === "awaiting_human_navigation"
          && this.#storedView(session).pendingAction?.type === "human_navigation"
        )
      )
      || (
        command.type === "steer"
  ...
  async #prepareStart(
    ...
    try {
      ...
      if (signal.aborted) signal.throwIfAborted();
      throw new ApplicationSessionServiceError(
        "APPLICATION_SOURCE_UNAVAILABLE",
        "Provide applicant-profile.md in your private candidate context before starting an application",
      );
    }
    let transcriptPdf: Uint8Array | undefined;
    try {
      transcriptPdf = validateTranscriptPdf(await this.#transcriptReader());
    } catch (error) {
      if (signal.aborted) signal.throwIfAborted();
    ...
    return {
      resumeRevision: run.currentRevision,
      jobUrl,
      opportunityKind: run.opportunityKind,
      autoSubmit: options.autoSubmit ?? run.autoSubmit,
      autoEnd: options.autoEnd ?? false,
      profile,
      ...(transcriptPdf === undefined ? {} : { transcriptPdf }),
      pdf,
      tailoredTex,
    };
  }
  ...
  async #resumeOnce(
    ...
    try {
      ...
      if (
        !currentPdf
        || prepared.resumeRevision !== session.resumeRevision
    ...
    try {
      await harness.create({
        ...
        autoEnd: prepared.autoEnd,
        personalInformationMarkdown: prepared.profile,
        resumePdf,
        resumeSource,
        ...(prepared.transcriptPdf === undefined
          ? {}
          : { transcriptPdf: prepared.transcriptPdf }),
      }, signal);
    } catch (error) {
      if (signal.aborted) signal.throwIfAborted();
