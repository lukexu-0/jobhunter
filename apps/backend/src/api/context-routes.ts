import { ContextMissingSourcesError } from "../context/service.ts";
  ...
  return async function routeContext(request: Request, url: URL): Promise<Response | null> {
    ...
    if (request.method === "POST" && url.pathname === "/v1/context/sync") {
      ...
      try {
        ...
        if (error instanceof ContextMissingSourcesError) return apiResponse.error("CONTEXT_SOURCES_MISSING", error.message, 409);
