...
import { ContextMissingSourcesError, ContextStaleError } from "../context";
import {
  PipelineRepository,
...
const FailedQaReportSchema = z.object({
  checks: z.array(z.object({
    id: DeterministicQaCheckIdSchema,
...
function missingContextSources(error: ContextMissingSourcesError): RunServiceError {
  const message = error.sourceIds.includes("resume-baseline")
    ? "Provide a resume baseline in your private candidate context before continuing"
    : "Provide the required candidate context sources before continuing";
  return new RunServiceError("CONTEXT_SOURCES_MISSING", message, 409);
}
...
function publicFilename(kind: ArtifactKind): string {
  if (kind === "compiled-pdf") return "Applicant_Resume.pdf";
  if (kind === "tailored-tex") return "Applicant_Resume.tex";
...
export class RunApplicationService {
  ...
  async listRuns(): Promise<RunDto[]> {
    ...
    const applicationStates = this.dependencies.repository.listApplicationSessionStates(
      runs.map(({ id }) => id),
    );
    return await Promise.all(
      runs.map((run) => this.#toDto(run, applicationStates.get(run.id) ?? null)),
    );
  }
  ...
  async retailorRun(id: string, expectedPdfSha256: string): Promise<RunDto> {
    return await this.#command(async () => {
      this.#assertArtifactsRetained(id);
      const current = sourceSnapshot(await this.#freshSnapshot());
      return this.dependencies.repository.retailor(id, expectedPdfSha256, current);
    });
  }
  ...
  async approveRun(id: string, expectedPdfSha256: string, acknowledgeVisualIssues: boolean): Promise<RunDto> {
    return await this.#command(() => {
      this.#assertArtifactsRetained(id);
      return this.dependencies.repository.approve(id, expectedPdfSha256, acknowledgeVisualIssues);
    });
  }
  ...
  async #freshSnapshot(): Promise<ContextSnapshot> {
    try {
      ...
      if (error instanceof ContextMissingSourcesError) throw missingContextSources(error);
  ...
  async #toDto(
    run: PublicRun,
    applicationState?: Pick<PublicApplicationSession, "bridgeState" | "generation" | "slotReleased"> | null,
  ): Promise<RunDto> {
    const repository = this.dependencies.repository;
    const latestApplication = applicationState === undefined
      ? repository.getLatestApplicationSession(run.id)
      : applicationState;
    ...
    let failureChecks: RunDto["failureChecks"];
    if (artifactsRetained && run.status === "failed" && run.failedStage === "deterministic_qa") {
      const report = repository.getArtifact(run.id, "deterministic-qa");
      if (report?.revision === run.currentRevision) {
        try {
          const verified = await readVerifiedArtifactBytes(this.dependencies.artifacts, report, MAX_PUBLIC_METADATA_BYTES);
          const parsed = FailedQaReportSchema.parse(JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(verified.bytes)));
          failureChecks = parsed.checks.filter(({ status }) => status === "fail").map(({ id }) => id);
        } catch {
          // Diagnostics must not hide the failed run when a report is missing or corrupt.
          // The UI falls back to the failed stage; never expose artifact bytes or errors.
        }
      }
    }
    return {
      ...
      isApplying: latestApplication !== null
        && ACTIVE_APPLICATION_SESSION_BRIDGE_STATES[latestApplication.bridgeState] === true,
      isApplicationSessionOpen: latestApplication !== null && !latestApplication.slotReleased,
      ...(latestApplication?.bridgeState === "failed"
        ? { applicationFailureGeneration: latestApplication.generation }
        : {}),
