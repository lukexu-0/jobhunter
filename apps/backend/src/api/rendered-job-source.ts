...
import { captureNativeChrome } from "./native-chrome-source.ts";
import {
...
const PROFILE_PREFIX = "jobhunt-rendered-job-";
...
export interface RenderedJobSource {
...
export class RenderJobSourceError extends Error {
  constructor() {
    super("Secure browser rendering failed. Check Chrome and browser containment support, or paste the opportunity details.");
    this.name = "RenderJobSourceError";
  }
}
...
function decodeCompleteHtmlDocument(output: CapturedOutput): string | undefined {
  if (output.truncated || output.bytes !== output.data.byteLength || output.data.byteLength === 0) return undefined;
  let dom: string;
  try {
  ...
  return hasCompleteHtmlDocument(dom) ? dom : undefined;
}

function hasCompleteHtmlDocument(dom: string): boolean {
  if (dom.includes("\0")) return false;
  if (/<body\b[^>]*\bclass=(["'])[^"']*\bneterror\b[^"']*\1/i.test(dom)) return false;
  const opening = /<html(?:\s|>)/i.exec(dom);
  const closing = /<\/html>\s*$/i.exec(dom);
  if (!opening || !closing || closing.index <= opening.index) return false;
  return hasValidHtmlPreamble(dom.slice(0, opening.index));
}
...
export async function renderJobSourceWithChrome(
...
): Promise<RenderedJobSource | undefined> {
  ...
  try {
    ...
    const native = process.platform === "darwin" && options.processBoundary === undefined;
    const args = [
      "--headless=new",
      ...(native ? [] : ["--dump-dom", `--virtual-time-budget=${CHROME_VIRTUAL_TIME_BUDGET_MS}`]),
    ...
    if (native) {
      rendered = await captureNativeChrome({
        url: renderUrl, cwd: profileDirectory, args, signal,
        timeoutMs: CHROME_TIMEOUT_MS, virtualTimeBudgetMs: CHROME_VIRTUAL_TIME_BUDGET_MS,
        stdoutLimit: CHROME_STDOUT_LIMIT, stderrLimit: CHROME_STDERR_LIMIT,
      });
      if (!hasCompleteHtmlDocument(rendered.html)) rendered = undefined;
    } else {
      const processResult = await runTrustedProcess({
        command: "google-chrome", args, cwd: profileDirectory,
        timeoutMs: CHROME_TIMEOUT_MS, signal,
        stdoutLimit: CHROME_STDOUT_LIMIT, stderrLimit: CHROME_STDERR_LIMIT,
        resourceProfile: BROWSER_RESOURCE_PROFILE.name,
      }, options.processBoundary);
      if (!processResult.aborted && !processResult.timedOut && processResult.code === 0
        && processResult.signal === null && !processResult.stderr.truncated) {
        const html = decodeCompleteHtmlDocument(processResult.stdout);
        if (html !== undefined) rendered = { html, finalUrl: null };
      }
    }
  } catch {
    rendered = undefined;
  ...
  if (cleanupFailed || rendered === undefined) throw new RenderJobSourceError();
  return rendered;
}
