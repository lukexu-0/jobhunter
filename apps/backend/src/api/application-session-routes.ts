...
export interface ApplicationSessionRouteService {
  ...
  openBrowser(runId: string, signal: AbortSignal): Promise<void>;
  command(
    runId: string,
  ...
  return async function routeApplicationSession(
    ...
    try {
      ...
      if (request.method === "POST" && segments.length === 4) {
        ...
        const { expectedApprovedPdfSha256 } = body.data;
        const options = {
          ...(body.data.autoSubmit === undefined ? {} : { autoSubmit: body.data.autoSubmit }),
          ...(body.data.autoEnd === undefined ? {} : { autoEnd: body.data.autoEnd }),
          ...(body.data.reapply === undefined ? {} : { reapply: body.data.reapply }),
        };
      ...
      if (
        ...
        if (request.body !== null) {
          return apiResponse.error(
            "INVALID_REQUEST",
            "Application browser request must be bodyless",
            400,
          );
        }
        await service.openBrowser(runId, request.signal);
