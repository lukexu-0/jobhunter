...
export const AUTH_PROVIDERS = ["openai-codex", "google-antigravity"] as const satisfies readonly AuthProvider[];
...
async function createStorage(): Promise<AuthStorageLike> {
  const configuredPath = process.env.JOBHUNT_AUTH_DATABASE ?? authDatabasePath;
