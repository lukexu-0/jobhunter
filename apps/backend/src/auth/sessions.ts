...
import { AUTH_PROVIDERS, assertProviderOAuthConnected, type AuthStorageLike } from "./storage";
...
export class AuthSessionManager {
  ...
  constructor(private readonly storage: AuthStorageLike, dependencies: AuthSessionDependencies = {}) {
    ...
    this.#providerLogin = dependencies.providerLogin
      ?? ((provider, controller) => {
        if (!(AUTH_PROVIDERS as readonly string[]).includes(provider)) throw new Error("OAuth provider is unavailable");
    ...
    this.#providerLogout = dependencies.providerLogout
      ?? ((provider) => (AUTH_PROVIDERS as readonly string[]).includes(provider) ? storage.logout(provider) : Promise.resolve());
  }
