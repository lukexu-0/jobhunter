...
export class ContextApplicationService implements ContextRouteService, RunContextSnapshotService {
  ...
  getContext(): ContextStatus {
    this.#requireOpen();
    return checkContextFreshness(this.#database, this.#currentManifest());
  }

  syncContext(): ContextStatus {
    this.#requireOpen();
    const loadedManifest = this.#currentManifest();
    const synchronized = synchronizeContext(this.#database, loadedManifest);
    const freshness = checkContextFreshness(this.#database, loadedManifest);
  ...
  createSnapshot(): ContextSnapshot {
    this.#requireOpen();
    const snapshot = createContextSnapshot(this.#database, this.#currentManifest());
  ...
  loadStageSourceContext(runId: string): StageSourceContext {
    ...
    const baselineDefinition = snapshot.sources.find((source) => source.kind === "baseline");
  ...
  #requireNoDrift(snapshot: ContextSnapshot): void {
    if (!verifyContextSnapshot(snapshot, this.#currentManifest()).valid) {
      throw new ContextStaleError("Context source drift detected; synchronize before continuing");
    }
  }

  #currentManifest(): LoadedContextManifest {
    return loadContextManifest(this.#loadedManifest.manifestPath, this.#loadedManifest.repositoryRoot);
  }
}
