...
function migrateSourceVersions(database: Database): void {
  const existing = database.query<{ sql: string }, []>("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'source_versions'").get();
  if (!existing?.sql.includes("UNIQUE(source_id, sha256)")) return;

  database.exec("PRAGMA foreign_keys = OFF");
  try {
    database.exec(`
      BEGIN IMMEDIATE;
      CREATE TABLE source_versions_v2 (
        id TEXT PRIMARY KEY,
        source_id TEXT NOT NULL,
        relative_path TEXT NOT NULL,
        kind TEXT NOT NULL CHECK (kind IN ('baseline', 'authoritative-markdown')),
      ...
      INSERT INTO source_versions_v2 SELECT * FROM source_versions;
      DROP TABLE source_versions;
      ALTER TABLE source_versions_v2 RENAME TO source_versions;
      COMMIT;
    `);
  } catch (error) {
...
export function openContextDatabase(
  path = process.env.JOBHUNT_CONTEXT_DATABASE ?? DEFAULT_CONTEXT_DATABASE_PATH,
): Database {
  ...
  migrateSourceVersions(database);
