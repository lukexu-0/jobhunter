...
export class ContextMissingSourcesError extends Error {
  readonly sourceIds: readonly string[];

  constructor(sourceIds: readonly string[]) {
    const guidance = sourceIds.includes("resume-baseline")
      ? "Add your resume at .jobhunt-data/user-info/resume-main/resume.tex"
      : "Add the context source files listed in apps/backend/context-sources.json";
    super(`${guidance} before synchronizing context (missing: ${sourceIds.join(", ")}).`);
    this.name = "ContextMissingSourcesError";
    this.sourceIds = Object.freeze([...sourceIds]);
  }
}

function existingSourcePaths(loaded: LoadedContextManifest): ReadonlyMap<string, string> {
  const paths = new Map<string, string>();
  const missing: string[] = [];
  for (const source of loaded.manifest.sources) {
    const path = resolveContextSourceIfPresent(loaded.repositoryRoot, source);
    if (path) paths.set(source.id, path);
    else missing.push(source.id);
  }
  if (missing.length) throw new ContextMissingSourcesError(missing);
  return paths;
}
...
export function syncContext(
  ...
  const paths = existingSourcePaths(loaded);
  const changedSources: string[] = [];
  const activeSourceIds = new Set(loaded.manifest.sources.map((source) => source.id));
  let blockCount = 0;
  const sync = database.transaction(() => {
    for (const source of loaded.manifest.sources) {
      const bytes = readFileSync(paths.get(source.id)!);
      const hash = sha256(bytes);
      const sourceVersionId = `source_${sha256(`${JSON.stringify(source)}\0${hash}`)}`;
      const head = database.query<HeadRow, [string]>(`
        ...
        WHERE h.source_id = ?
      `).get(source.id);
      if (head?.source_version_id !== sourceVersionId) changedSources.push(source.id);
...
export function checkContextFreshness(
  ...
  for (const source of loaded.manifest.sources) {
    const path = resolveContextSourceIfPresent(loaded.repositoryRoot, source);
    if (!path) { missingSources.push(source.id); continue; }
    ...
    `).get(source.id);
    if (!head || head.sha256 !== currentHash) staleSources.push(source.id);
  }
...
export function createContextSnapshot(
  ...
  const absent = loaded.manifest.sources.filter((source) => !resolveContextSourceIfPresent(loaded.repositoryRoot, source)).map((source) => source.id);
  if (absent.length) throw new ContextMissingSourcesError(absent);
...
export function verifyContextSnapshot(
  ...
  for (const source of loaded.manifest.sources) {
    const path = resolveContextSourceIfPresent(loaded.repositoryRoot, source);
    if (!path || snapshot.sourceHashes[source.id] !== sha256(readFileSync(path))) changedSources.push(source.id);
  }
