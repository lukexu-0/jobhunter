...
export const ANALYSIS_TASK =
  "Identify truthful JD keywords and exact replacements for existing resume bullets and skills.";
export const ANALYSIS_INSTRUCTIONS =
  "Use supplied JD, ATS extraction, baseline inventory, and candidate source facts only; never fabricate qualifications, metrics, or outcomes. Follow relevant source instructions for the entity and its explicit equivalents, but instructions are not facts. Use JD keywords only when supported by the candidate sources. Always preserve impact when performing edits. Write clearly for recruiters and technical staff with conventional terms, never \"Agentic workflow systems\". Make each project's first bullet a summary. Use \"Accomplished [X] as measured by [Y] by doing [Z]\" only when the sources support X, Y, and Z. Copy hashes and call submit_job_analysis once.";
...
function semanticValidationIssue(error: unknown): AnalysisValidationIssue {
  ...
  return { path: "$", message: "must satisfy the supplied hashes, baseline, and JD constraints" };
}
...
function finishAnalysisValidationFeedback(issueLines: readonly string[]): string {
  const feedback = [
    ...
    "- JD quotes must be verbatim and keyword identities must match the supplied ATS extraction.",
  ].join("\n");
...
export async function runAnalysisAgent(attempt: AnalysisAgentAttempt): Promise<JobAnalysis> {
  ...
  }).map(({ sourceId, entityId, headingPath, text, caveats }) => ({
    source: sourceById.get(sourceId)!.displayName,
    entityId,
    headingPath,
    text,
    caveats,
