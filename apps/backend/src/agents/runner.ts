...
import { CodexCompactionError, type CodexCompactionDiagnostic } from "../models/oauth-codex-model.ts";
...
function modelTraceError(
  ...
  try {
    ...
    return {
      ...
      ...(code === undefined ? {} : { code }),
      ...(value instanceof CodexCompactionError ? { compaction: value.compaction } : {}),
      ...(cause === undefined
        ? {}
