...
const EDIT_INSTRUCTIONS =
  "Treat comments and QA findings as requirements, not candidate facts. Follow relevant source instructions for the entity and its explicit equivalents. Use only supplied candidate source facts for claims; never fabricate qualifications, metrics, or outcomes. Preserve immutable analysis and the current tailoringWorkflowSha256, produce a plan rather than TeX, disposition every human comment, and call submit_edit_plan exactly once.";
export async function runEditAgent(attempt: EditAgentAttempt): Promise<EditResult> {
  ...
  const baseline = attempt.input.context.evidence
    .filter((block) => sourceById.get(block.sourceId)?.kind === "baseline")
    .map(({ entityId, headingPath, text, caveats }) => ({ entityId, headingPath, text, caveats }));
