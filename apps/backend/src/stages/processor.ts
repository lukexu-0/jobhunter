...
function mustIncludeEntities(snapshot: ContextSnapshot): ReadonlySet<string> {
  const protectedEntities = new Set(snapshot.mustIncludeDirectives.map((directive) => directive.entityId));
  if (protectedEntities.size === 0) return protectedEntities;
  const bindings = new Map<string, Set<string>>();
  const connect = (left: string, right: string): void => {
    if (!bindings.has(left)) bindings.set(left, new Set());
    if (!bindings.has(right)) bindings.set(right, new Set());
    bindings.get(left)!.add(right);
    bindings.get(right)!.add(left);
  };
  for (const [left, right] of Object.entries(snapshot.explicitEntityBindings)) connect(left, right);
  for (const source of snapshot.sources) {
    for (const baselineEntityId of source.baselineEntityIds) connect(source.entityId, baselineEntityId);
  }
  for (const entityId of protectedEntities) {
    for (const equivalent of bindings.get(entityId) ?? []) protectedEntities.add(equivalent);
  }
...
function validateOnePageCorrectionPlan(
  ...
  for (const [index, candidate] of expected.entries()) {
    ...
    if (!omission || omission.baselineItemId !== candidate.baselineItemId) {
...
export class PipelineStageProcessor {
  ...
  async #tailor(claim: RunClaim, run: PublicRun, attempt: PublicAttempt, sources: StageSourceContext, signal: AbortSignal, audit: AttemptAudit, agentRuntime: AgentRuntimeDependencies): Promise<void> {
    ...
    const { analysis } = parsePersistedJobAnalysis(await this.#readJson(analysisArtifact));
  ...
  async #edit(claim: RunClaim, run: PublicRun, attempt: PublicAttempt, sources: StageSourceContext, signal: AbortSignal, audit: AttemptAudit, agentRuntime: AgentRuntimeDependencies): Promise<void> {
    ...
    const storedAnalysis = parsePersistedJobAnalysis(await this.#readJson(analysisArtifact));
    const { analysis } = storedAnalysis;
    const currentPlan = parsePersistedTailoringPlan(await this.#readJson(planArtifact), storedAnalysis);
  ...
  async #deterministic(claim: RunClaim, run: PublicRun, attempt: PublicAttempt, sources: StageSourceContext, signal: AbortSignal, audit: AttemptAudit): Promise<void> {
    ...
    if (pureOnePageFailure && hasPositiveOverflowLineCount && hasValidPageExcess) {
      ...
      if (priorFailureCount < MAX_ONE_PAGE_CORRECTIONS) {
        const { analysis } = parsePersistedJobAnalysis(
  ...
  async #onePageCorrection(
    ...
    if (candidates.length === 0) {
      throw new Error("one-page correction has no eligible omission candidates");
    }
    return {
  ...
  #onePageCorrectionCandidates(
    ...
    const protectedEntityIds = mustIncludeEntities(sources.snapshot);
    return baseline.bullets
      .map((bullet, index) => {
        if (protectedEntityIds.has(bullet.entityId)) return undefined;
        return {
      ...
      .map(({ baselineItemId, section, entityId, text }) => ({
        baselineItemId,
        section,
        entityId,
        text,
