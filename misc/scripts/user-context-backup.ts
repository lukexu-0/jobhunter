...
const LOCKF_CONFLICT_EXIT_CODE = 75;
const OPERATION_LOCK_NAME = ".operation.lock";
const LEGACY_ROOT_DOSSIER_PATH = "jobhunt-resume-info.md";
const LEGACY_USER_INFO_PATH = "apps/user-info";
const PREVIOUS_USER_INFO_PATH = "user-info";
const USER_INFO_PATH = ".jobhunt-data/user-info";
const LEGACY_USER_INFO_PREFIX = `${LEGACY_USER_INFO_PATH}/`;
const PREVIOUS_USER_INFO_PREFIX = `${PREVIOUS_USER_INFO_PATH}/`;
const USER_INFO_PREFIX = `${USER_INFO_PATH}/`;
...
interface RestoreUserContextDependencies {
  readonly publishReplacement: (source: string, target: string) => Promise<void>;
}

const DEFAULT_RESTORE_DEPENDENCIES: RestoreUserContextDependencies = {
  publishReplacement: rename,
};
...
function validateSnapshotPath(path: string): void {
  ...
  if (
    path !== LEGACY_ROOT_DOSSIER_PATH
    && !path.startsWith(USER_INFO_PREFIX)
    && !path.startsWith(LEGACY_USER_INFO_PREFIX)
    && !path.startsWith(PREVIOUS_USER_INFO_PREFIX)
  ) {
    throw new Error("Snapshot manifest path is outside private user context");
  }
}

function relocateLegacySnapshotFile(file: UserContextSnapshotFile): UserContextSnapshotFile {
  const legacyPrefix = file.path.startsWith(LEGACY_USER_INFO_PREFIX)
    ? LEGACY_USER_INFO_PREFIX
    : file.path.startsWith(PREVIOUS_USER_INFO_PREFIX)
      ? PREVIOUS_USER_INFO_PREFIX
      : undefined;
  if (legacyPrefix === undefined) return file;
  return {
    ...file,
    path: USER_INFO_PREFIX + file.path.slice(legacyPrefix.length),
  };
}
...
async function withOperationLock<T>(
  ...
  if (readiness.done || new TextDecoder().decode(readiness.value) !== "locked\n") {
    ...
    if (exitCode === LOCKF_CONFLICT_EXIT_CODE) {
...
export async function backupUserContextForLaunch(
  ...
  const context = await resolveBackupContext(mode, appsRoot, environment);
  const userInfoRoot = join(context.checkoutRoot, ...USER_INFO_PATH.split("/"));
  const dataRoot = dirname(userInfoRoot);
  if (await lstatIfPresent(dataRoot) !== undefined) {
    await requireRealDirectory(dataRoot, "User-context data root");
  }
  if (await lstatIfPresent(userInfoRoot) === undefined) return undefined;
  return createUserContextSnapshot({ mode, appsRoot, environment, reason: "stable-start", ...controls });
}
...
async function stageVerifiedRestore(
  ...
  assertScopeMatches(manifest.scope, context.scope);
  const restoreFiles = manifest.files
    .map((source) => ({ source, target: relocateLegacySnapshotFile(source) }))
    .sort((left, right) => compareText(left.target.path, right.target.path));
  for (let index = 1; index < restoreFiles.length; index += 1) {
    if (restoreFiles[index - 1]?.target.path === restoreFiles[index]?.target.path) {
      throw new Error("Snapshot manifest paths collide after user-info migration");
    }
  }
  ...
  try {
    for (const { source: sourceFile, target } of restoreFiles) {
      const source = await requirePrivateSnapshotFile(snapshotFilesRoot, sourceFile);
      const destination = sourcePathForManifest(filesRoot, target.path);
      for (const directory of await ensurePrivateParentDirectories(filesRoot, destination)) {
        directories.add(directory);
      }
      await copyRegularFile(source, destination, DEFAULT_LIMITS, target);
      directories.add(dirname(destination));
    }
    for (const directory of [...directories].sort((left, right) => right.length - left.length)) {
      await syncDirectory(directory);
    }
    const restoredManifest = restoreFiles.every(({ source, target }) => source === target)
      ? manifest
      : { ...manifest, files: restoreFiles.map(({ target }) => target) };
    return { stageRoot, manifest: restoredManifest };
  } catch (error) {
...
export async function restoreUserContextSnapshot(
  ...
  dependencies: RestoreUserContextDependencies = DEFAULT_RESTORE_DEPENDENCIES,
    ...
    try {
      ...
      try {
        for (const replacement of replacements) {
          await validateRestoreTarget(context.checkoutRoot, replacement.file);
          await dependencies.publishReplacement(replacement.temporaryPath, replacement.targetPath);
