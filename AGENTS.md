...
6. The Python harness listens on `127.0.0.1:8765`, owns browser/session/human-gate state, and calls only the bearer-protected Bun `/v1/internal/application-agent` boundary. The per-run `autoSubmit` choice is persisted by the pipeline and is forwarded as `auto_submit` (`"true"` or `"false"`) in multipart form, then as `autoSubmit` in the strict internal request. `skipReview` controls only clean resume approval and automatic application start; it is not sent as model-visible task evidence. Auto-submit keeps origin, navigation, additional-information, mismatch, submission-ledger, one-shot action, and submission-result safeguards. Do not import across this Python/TypeScript boundary or expose the internal route to browser code.
