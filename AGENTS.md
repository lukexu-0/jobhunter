...
2. Run creation validates and safely loads an HTTP(S) opportunity, determines `job`, `hackathon`, `competition`, `event`, or `networking_event`, requires a fresh candidate-context snapshot, writes the input artifact, and creates the queued run transactionally.
